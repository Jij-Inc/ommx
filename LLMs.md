...
```{note}
Currently, an implementation guide for the Adapter (OMMX Sampler Adapter) to execute Sampler from OMMX is also being prepared.
```

This document outlines the recommendations for implementing an Adapter (hereafter called OMMX Solver Adapter) to execute Solver from OMMX. By following the guidelines below when implementing the OMMX Solver Adapter, you can maintain a certain level of consistency with other OMMX Solver Adapters, providing a better user experience for users utilizing multiple OMMX Solver Adapters. Therefore, the OMMX community recommends adhering to the following content.
