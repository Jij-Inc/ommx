"""OpenJij-specific preparation planning and materialization."""
...
class _OpenJijPreparationSupport:
    """Internal support for explicit OpenJij input preparation."""

    @classmethod
    def _check_prepared_input_applicability(
        cls, ommx_instance: Instance
    ) -> AdapterApplicabilityReport:
        """Adapter-owned callback for validating the produced input."""
    ...
    def _materialize_preparation(
        ...
        *,
        ...
        input_applicability = cls._check_prepared_input_applicability(working)
        report = OpenJijPreparationReport(
            source_check=source_check,
            steps=tuple(steps),
