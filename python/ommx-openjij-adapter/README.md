...
example, prepare a copy with a selected fixed penalty weight and use the strict
API for a constrained model:
...
sample_set = OMMXOpenJijSAAdapter.sample_strict(
    prepared,
    num_reads=16,
)
...
Pass the prepared `Instance` to `sample_strict`. It remains the evaluation
