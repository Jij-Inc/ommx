...
The central workflow is to create an `Experiment`, attach experiment-wide
context, create one `Run` per comparison condition, and call `Run.log_solve`
for each solver invocation. The whole experiment is committed as one immutable
`ommx.artifact.Artifact`.
...
The committed experiment can be loaded back as a read-only view. A `Solve`
stores descriptors for the input instance and output solution; use the parent
artifact to read the payloads.
