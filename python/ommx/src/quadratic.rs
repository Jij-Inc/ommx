...
pyo3_stub_gen::inventory::submit! {
    pyo3_stub_gen::derive::gen_methods_from_python! {
        ...
        class Quadratic:
            @overload
            def __add__(self, rhs: ScalarLike | LinearLike | Parameter | Quadratic) -> Quadratic: ...
            @overload
            ...
            @overload
            def __radd__(self, lhs: ScalarLike | LinearLike | Parameter | Quadratic) -> Quadratic: ...
            @overload
            ...
            @overload
            def __sub__(self, rhs: ScalarLike | LinearLike | Parameter | Quadratic) -> Quadratic: ...
            @overload
            ...
            @overload
            def __rsub__(self, lhs: ScalarLike | LinearLike | Parameter | Quadratic) -> Quadratic: ...
            @overload
...
impl Quadratic {
    ...
    pub fn new(
        ...
        // Vector lengths are a Python argument-shape contract, not a Rust SDK
        // domain failure, so keep this validation outside the shared mapper.
    ...
    pub fn almost_equal(&self, other: &Quadratic, atol: f64) -> PyResult<bool> {
    ...
    pub fn py_add(&self, py: Python<'_>, rhs: crate::FunctionInput) -> PyResult<Py<PyAny>> {
        Ok(match rhs {
            ...
            crate::FunctionInput::Scalar(Some(c)) => {
                Quadratic(crate::error::map_coefficient(&self.0 + c)?)
                    .into_pyobject(py)?
                    .into_any()
                    .unbind()
            }
            ...
            crate::FunctionInput::Quadratic(q) => {
                Quadratic(crate::error::map_coefficient(&self.0 + &q)?)
                    .into_pyobject(py)?
                    .into_any()
                    .unbind()
            }
    ...
    pub fn py_sub(&self, py: Python<'_>, rhs: crate::FunctionInput) -> PyResult<Py<PyAny>> {
        Ok(match rhs {
            ...
            crate::FunctionInput::Linear(l) => {
                Quadratic(crate::error::map_coefficient(self.0.clone() - &l)?)
                    .into_pyobject(py)?
                    .into_any()
                    .unbind()
            }
    ...
    pub fn add_assign(&mut self, rhs: &Quadratic) -> PyResult<()> {
    ...
    #[gen_stub(skip)]
    pub fn __iadd__(&mut self, rhs: &Quadratic) -> PyResult<()> {
        self.add_assign(rhs)
    }
    ...
    pub fn py_mul(&self, py: Python<'_>, rhs: crate::FunctionInput) -> PyResult<Py<PyAny>> {
        Ok(match rhs {
            crate::FunctionInput::Scalar(None) => Quadratic(ommx::Quadratic::default())
                ...
                .unbind(),
            crate::FunctionInput::Scalar(Some(c)) => {
                Quadratic(crate::error::map_coefficient(self.0.clone() * c)?)
                    .into_pyobject(py)?
                    .into_any()
                    .unbind()
            }
            crate::FunctionInput::Linear(l) => {
                Polynomial(crate::error::map_coefficient(&self.0 * &l)?)
                    .into_pyobject(py)?
                    .into_any()
                    .unbind()
            }
    ...
    pub fn add_scalar(&self, scalar: f64) -> PyResult<Quadratic> {
        match TryInto::<Coefficient>::try_into(scalar) {
            Ok(coeff) => Ok(Quadratic(crate::error::map_coefficient(&self.0 + coeff)?)),
    ...
    pub fn add_linear(&self, linear: &Linear) -> PyResult<Quadratic> {
        Ok(Quadratic(crate::error::map_coefficient(
            &self.0 + &linear.0,
        )?))
    }

    pub fn mul_scalar(&self, scalar: f64) -> PyResult<Quadratic> {
        match TryInto::<Coefficient>::try_into(scalar) {
            Ok(coeff) => Ok(Quadratic(crate::error::map_coefficient(
                self.0.clone() * coeff,
            )?)),
    ...
    pub fn mul_linear(&self, linear: &Linear) -> PyResult<Polynomial> {
        Ok(Polynomial(crate::error::map_coefficient(
            &self.0 * &linear.0,
        )?))
    }
