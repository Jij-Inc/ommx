use crate::{next_constraint_id, Constraint, DecisionVariable, Polynomial, Quadratic, Rng};
...
impl Linear {
    ...
    #[pyo3(signature = (state, *, atol=None))]
    pub fn partial_evaluate(&self, state: &Bound<PyBytes>, atol: Option<f64>) -> Result<Linear> {
        use ommx::Message;
        let state = ommx::v1::State::decode(state.as_bytes())?;
        let atol = match atol {
            Some(value) => ommx::ATol::new(value)?,
