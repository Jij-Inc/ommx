...
pub fn preflight_function(lhs: &Function, rhs: &Function) -> Result<(), CoefficientError> {
    match (lhs, rhs) {
        ...
        // Adding an expression constructs an ordered n-ary Add node instead of
        // merging polynomial coefficient maps, so there is no coefficient
        // arithmetic to preflight.
        (Function::Unary(_) | Function::Nary(_) | Function::Binary(_), _)
        | (_, Function::Unary(_) | Function::Nary(_) | Function::Binary(_)) => Ok(()),
    }
}
