...
such as JijModeling. It has its own Cargo workspace and committed lockfile. Its
Rust SDK and bridge dependencies are the published `=3.0.0-beta.6` crates from
crates.io. Its Python dependency is `ommx==2.9.0`.
...
The `bridge-v1` workflow builds this producer and Python SDK 2.9 as separate
wheels and installs both into an isolated Python 3.12 environment. It checks
...
PYO3_PYTHON="$PWD/.venv-bridge/bin/python" cargo run --locked --manifest-path python/bridge-tests/modeling/Cargo.toml --bin stub_gen
...
The tests live outside normal SDK and benchmark discovery. The dedicated job
requires the producer package; a missing or broken installation fails the job.
