...
def test_neos_2626858_aoos_variable_domains(loader):
    ...
    assert sum(v.kind == DecisionVariable.BINARY for v in variables) == 209
    assert sum(v.kind == DecisionVariable.INTEGER for v in variables) == 315
    ...
    for variable in affected:
        assert variable.kind == DecisionVariable.BINARY, variable.name
