...
- When `cargo-public-api` and a nightly toolchain are available, produce the
  ...
  justification.
- The diff grep only sees source text. Items generated by macros — PyO3
  `#[pymethods]`, derive macros — never appear in it. `cargo public-api`
  (rustdoc JSON) and rust-analyzer queries see the post-expansion API, so use
  one of them when the change touches macro-heavy code.
...
1. Check that visibility follows the domain owner.
   ...
   - When rust-analyzer is available, answer "which callers genuinely need
     access" with evidence: `findReferences` on the item gives the actual
     ...
     exceeds their actual reachability.
