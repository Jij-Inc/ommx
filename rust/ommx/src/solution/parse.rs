...
impl Parse for crate::v1::Solution {
    ...
    fn parse(self, _: &Self::Context) -> Result<Self::Output, ParseError> {
        ...
        let metadata = SolutionMetadata {
            optimality: self.optimality.try_into().map_err(|_| RawParseError::UnspecifiedEnum {
                enum_name: "ommx.v1.Optimality",
            })?,
            relaxation: self.relaxation.try_into().map_err(|_| RawParseError::UnspecifiedEnum {
...
impl Parse for crate::v1::SampleSet {
    ...
    fn parse(self, _: &Self::Context) -> Result<Self::Output, ParseError> {
        ...
        let sense = self.sense.try_into().map_err(|_| RawParseError::UnspecifiedEnum {
...
impl From<SampleSet> for crate::v1::SampleSet {
    fn from(sample_set: SampleSet) -> Self {
        ...
        let objectives = sample_set.objectives()
            .as_ref()
            .map(|obj| obj.clone().into());
        let constraints = sample_set.constraints()
            .iter()
            .map(|sc| sc.clone().into())
            .collect();
        let feasible_relaxed: std::collections::HashMap<u64, bool> = sample_set.feasible_relaxed().clone().into_iter().collect();
        let feasible: std::collections::HashMap<u64, bool> = sample_set.feasible().clone().into_iter().collect();
...
mod tests {
    ...
    fn test_sample_set_parse() {
        let v1_sample_set = v1::SampleSet {
            ...
            feasible: [(0, true), (1, false), (2, false)].iter().cloned().collect(),
