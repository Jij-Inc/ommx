...
//!   through [`LocalRegistry`], which re-derives sha256 and asserts the
//!   result matches the tar-entry path encoding.
...
/// [`ArchiveInspectView::read`] without touching the SQLite Local Registry.
#[derive(Debug, Clone)]
#[non_exhaustive]
...
/// First-pass helper for [`ArchiveInspectView::read`]: stream the tar to find
...
/// Second-pass helper for [`ArchiveInspectView::read`]: stream the tar a
...
impl LocalRegistry {
    /// Import a `.ommx` OCI archive on disk into this Local Registry.
    ///
    /// Streams the archive entries once: writes every `blobs/sha256/<digest>`
    /// blob through the registry, captures `oci-layout` + `index.json` into
    /// memory for the post-pass parse, and publishes the manifest ref.
    ///
    /// Unnamed archives are imported under a freshly synthesized anonymous
    /// ref so the artifact is addressable.
    pub fn import_oci_archive(&self, path: &Path) -> Result<OciDirImport> {
        ...
        // The manifest blob is now resident in the registry (it was a
        // `blobs/sha256/<digest>` entry in the tar). Read it back to discover
        // the layers; this re-verifies its digest in the process.
        ...
        // All referenced blobs are already in the registry (written during the
        // tar scan). Verify the manifest is self-contained before publishing the
        // ref descriptor.
...
/// Single tar pass: write blob entries to the registry CAS, capture
...
/// Verify a manifest-referenced blob is present in the registry.
