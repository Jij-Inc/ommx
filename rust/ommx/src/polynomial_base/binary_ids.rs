...
/// ID sequence for HUBO problems.
///
/// Unlike `BinaryIds`, this allows repeating ids.
#[derive(Debug, Clone, PartialOrd, Ord, PartialEq, Eq)]
pub struct BinaryIdSeq(Vec<u64>);

impl From<Vec<u64>> for BinaryIdSeq {
...
impl From<MonomialDyn> for BinaryIdSeq {
    fn from(ids: MonomialDyn) -> Self {
        Self::from(
            ids.into_inner()
                .into_iter()
...
impl Serialize for BinaryIdSeq {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut tup = serializer.serialize_tuple(self.0.len())?;
        for id in &self.0 {
            tup.serialize_element(id)?;
