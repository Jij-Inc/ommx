...
Remote manifest lookup and Local Registry pulls return the OMMX-owned,
non-exhaustive [`RemoteArtifactError`](crate::artifact::RemoteArtifactError).
Callers can distinguish an absent manifest from authentication, authorization,
transport, and invalid-Artifact failures without depending on `oci-client` or
matching error strings. Each variant retains the requested
[`ImageRef`](crate::artifact::ImageRef) and the original source chain.
