...
summaries for Experiment refs without opening each Experiment artifact. The
listing is backed by a digest-addressed SQLite manifest cache plus an
Experiment-config projection. These projection rows are written in the same
transaction as ref updates on commit, archive import, OCI directory import, and
remote pull paths.
