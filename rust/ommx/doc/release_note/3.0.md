...
overview for users upgrading from 2.6.1, then reserves the final section for
short updates within the 3.0.x line.
...
Related PRs: [#983](https://github.com/Jij-Inc/ommx/pull/983),
...
Related PR: [#969](https://github.com/Jij-Inc/ommx/pull/969).
...
Related PR: [#800](https://github.com/Jij-Inc/ommx/pull/800).

## 3.0.x updates

Append user-visible updates within the 3.0 release line here. Each entry should
...
[`FinishedSampleRecord`](crate::experiment::FinishedSampleRecord) record a
sampler call as `Instance -> SampleSet` without reducing the output to one
solution. The call is stored separately from solver calls as a
[`Sampling`](crate::experiment::Sampling), and a successful call is represented
by [`SamplingStatus::Finished`](crate::experiment::SamplingStatus::Finished)
even when the SampleSet has no feasible entry. Solver calls remain
[`Solve`](crate::experiment::Solve) records whose output is an optional
[`Solution`](crate::Solution).

Adapter diagnostics shared by Solve and Sampling records use
[`AdapterDiagnosticPayload`](crate::experiment::AdapterDiagnosticPayload),
renamed from the Solve-specific `SolveDiagnosticPayload`.

### Legacy v1 constraint hints remain advisory ([#1058](https://github.com/Jij-Inc/ommx/pull/1058))

[`Instance::from_v1_bytes`](crate::Instance::from_v1_bytes) and
[`ParametricInstance::from_v1_bytes`](crate::ParametricInstance::from_v1_bytes)
now ignore legacy v1 `ConstraintHints` while preserving the referenced regular
constraints, removed state, and constraint context. Even structurally plausible
hints are not promoted automatically, so unverified metadata cannot change the
feasible set or required capabilities. Because deserialization no longer creates
first-class special constraints implicitly, imported instances can round-trip
through the v1 bytes APIs.

Construct first-class special constraints from trusted modeling input rather
than from a legacy hint alone.

### Referenced blob size for Artifact refs ([#1056](https://github.com/Jij-Inc/ommx/pull/1056))

[`ArtifactRefRecord::referenced_blob_size`](crate::artifact::local_registry::ArtifactRefRecord::referenced_blob_size)
calculates the original Manifest JSON byte length plus its config size and
unique layer sizes from the parsed
[`ArtifactRefRecord::manifest`](crate::artifact::local_registry::ArtifactRefRecord::manifest).
`ArtifactRefRecord` is an immutable, digest-validated snapshot; its ref identity,
original Manifest size, and parsed Manifest are exposed through read-only getters.
The `ommx size <IMAGE_NAMES>...` CLI reports the same logical size for one or more
local Artifact or Experiment refs without reading payload CAS files. OCI
`subject` descriptors are excluded. Blobs shared by different refs are counted
for each ref, so these values are not additive physical Local Registry usage.

### Transparent attachment compression and streaming writes ([#1054](https://github.com/Jij-Inc/ommx/pull/1054))

[`Compression`](crate::experiment::Compression) adds opt-in zstd storage for
Experiment and Run attachments. Use
[`AttachmentLogger::log_attachment_compressed`](crate::experiment::AttachmentLogger::log_attachment_compressed)
or
[`AttachmentLogger::log_file_compressed`](crate::experiment::AttachmentLogger::log_file_compressed)
to compress a stored OCI layer. Attachment media-type, blob, typed, and file
readers use a reserved compression annotation to identify compressed layers,
then transparently remove the `+zstd` suffix and decompress the payload. An
unmarked logical media type ending in `+zstd` is preserved unchanged.

[`AttachmentLogger::log_attachment_from_reader`](crate::experiment::AttachmentLogger::log_attachment_from_reader)
streams a caller-provided reader into the Local Registry while computing the
content digest and size. Existing `log_file` calls use the same streaming path.

### Local Registry ref deletion and Experiment retention ([#1053](https://github.com/Jij-Inc/ommx/pull/1053))

[`LocalRegistry::remove_image_ref`](crate::artifact::local_registry::LocalRegistry::remove_image_ref)
removes a mutable image ref without deleting immutable manifest, config, or
layer blobs. Those blobs remain available to other refs and can be reclaimed by
the existing Local Registry GC once unreachable. The `ommx rm` CLI keeps ref
deletion separate from `ommx gc --delete` and reports this storage lifecycle
after deletion.

[`LocalRegistry::restore_image_ref`](crate::artifact::local_registry::LocalRegistry::restore_image_ref)
re-publishes a removed ref from the Manifest digest returned by
`remove_image_ref`. Restore validates the complete Manifest closure from the
CAS, serializes validation and publication against deleting GC passes, and
reports a conflict instead of replacing a ref that now points to a different
digest. Restoring an Experiment republishes its validated listing projection in
the same SQLite transaction as the ref. The CLI prints the corresponding
`ommx restore-ref` command after each deletion.

[`AnonymousRefOptions`](crate::artifact::local_registry::AnonymousRefOptions)
extends anonymous pruning to Experiment refs and age-based retention through
[`LocalRegistry::list_anonymous_refs`](crate::artifact::local_registry::LocalRegistry::list_anonymous_refs)
and
[`LocalRegistry::prune_anonymous_refs`](crate::artifact::local_registry::LocalRegistry::prune_anonymous_refs).
Pruning uses compare-and-delete semantics so a concurrently replaced ref is not
removed from a stale candidate snapshot.

### Configurable Experiment autosave frequency ([#1052](https://github.com/Jij-Inc/ommx/pull/1052))

[`AutosavePolicy`](crate::experiment::AutosavePolicy) lets unsealed
[`Experiment`](crate::experiment::Experiment) and
[`ExperimentDyn`](crate::experiment::ExperimentDyn) sessions checkpoint after
every closed Run, every N additional Runs, at a minimum time interval, or not
at all. The default remains `EveryRunClose`. Minimum-interval policies
rate-limit publish attempts, including failed attempts that may already have
written content-addressed blobs before the ref update failed.

The policy is runtime-only and is reset to the default for new, restored, and
forked sessions. It controls rolling `draft` checkpoints after Run close;
explicit failed and interrupted checkpoints remain available regardless of the
policy.

### Artifact and Experiment listing from the Local Registry ([#1029](https://github.com/Jij-Inc/ommx/pull/1029))

[`LocalRegistry::list_artifacts`](crate::artifact::local_registry::LocalRegistry::list_artifacts)
now returns [`ArtifactRefRecord`](crate::artifact::local_registry::ArtifactRefRecord)
values for every OMMX Artifact ref. Each record includes the Manifest and
Config digests, `artifactType`, update timestamp, Manifest annotations, and the
complete OCI Manifest JSON. Internal Experiment checkpoint refs are excluded
from this user-facing catalog by default; use
[`ArtifactListOptions`](crate::artifact::local_registry::ArtifactListOptions)
with `include_internal` for registry diagnostics.

[`LocalRegistry::list_experiments`](crate::artifact::local_registry::LocalRegistry::list_experiments)
now returns [`ExperimentRefRecord`](crate::artifact::local_registry::ExperimentRefRecord)
values for Experiment refs. Each record additionally includes common
status/count fields and the complete Experiment Config JSON. Both listings are
backed by digest-addressed SQLite caches of the original Manifest and Config
JSON bytes. Missing rows are backfilled from the CAS, and cache rows that become
unreachable after ref replacement or deletion are removed.

[`LocalRegistry::list_experiment_checkpoints`](crate::artifact::local_registry::LocalRegistry::list_experiment_checkpoints)
returns recoverable `draft`, `failed`, and `interrupted` checkpoints by their
original requested image name. The corresponding `*_with_options` listing APIs
return [`RegistryListReport`](crate::artifact::local_registry::RegistryListReport)
values with structured per-ref warnings. In default best-effort mode, invalid
cache entries are repaired from the CAS when possible and otherwise skipped,
and malformed individual ref identities are also skipped. The option types
expose `strict` mode for callers that require the first individual failure to
abort the listing. SQLite schema, query, and cache-write failures always abort
the listing.

Opening a version 1 Local Registry migrates it to version 2 in place while
preserving refs and the registry ID. [`RefRecord`](crate::artifact::local_registry::RefRecord)
now exposes `manifest_digest` instead of a reconstructed OCI descriptor because
refs no longer duplicate Manifest media type, size, or annotations.

[`Experiment::set_annotation`](crate::experiment::Experiment::set_annotation)
and [`ExperimentDyn::set_annotation`](crate::experiment::ExperimentDyn::set_annotation)
store caller-owned manifest annotations on committed Experiment artifacts, so
registry listings can carry workflow metadata such as upstream problem names or
dashboard grouping keys. OMMX-reserved annotation keys remain rejected.

### Consuming partial evaluation for `Instance` ([#1034](https://github.com/Jij-Inc/ommx/pull/1034))

[`Instance::into_partial_evaluated`](crate::Instance::into_partial_evaluated)
is now available as the consuming counterpart to
[`Evaluate::partial_evaluate`](crate::Evaluate::partial_evaluate). It applies
the same partial-evaluation rewrite and returns the rewritten
[`Instance`](crate::Instance).

Use [`Evaluate::partial_evaluate`](crate::Evaluate::partial_evaluate) when a
borrowed instance must remain unchanged on error. Use
[`Instance::into_partial_evaluated`](crate::Instance::into_partial_evaluated)
when the caller can give up the original instance on error and wants the owned
rewrite path.
...
`application/org.ommx.v1.artifact` type. Experiment loading validates this root
artifact type before decoding the Experiment config, so a generic
[`LocalArtifact`](crate::artifact::LocalArtifact) is not interpreted as an
Experiment merely because its config descriptor uses the Experiment config
media type.

The Rust SDK also exposes
[`media_types::is_ommx_artifact_type`](crate::artifact::media_types::is_ommx_artifact_type)
for callers that need to classify OCI Manifest `artifactType` values without
hard-coding the current OMMX artifact type set.
...
[`Instance::unary_encode`](crate::Instance::unary_encode) is now available as
an alternative to [`Instance::log_encode`](crate::Instance::log_encode) for
finite integer variables. For an integer variable `x` in `[lower, upper]`, it
introduces `upper - lower` binary variables and substitutes
`x = lower + sum(b)`.

The generated binary variables use the `ommx.unary_encode` modeling label with
the original variable ID and unary bit index as subscripts. Since the encoding
size grows linearly with the integer range width, it is intended for narrow
ranges where local-move samplers benefit from single-bit moves changing the
decoded value by one. To avoid accidental large allocations,
[`Instance::unary_encode`](crate::Instance::unary_encode) takes an explicit
`max_range` and rejects variables whose range width exceeds it. Use
[`Instance::DEFAULT_UNARY_ENCODING_MAX_RANGE`](crate::Instance::DEFAULT_UNARY_ENCODING_MAX_RANGE)
as the standard guard unless a different limit is intentional.

[`Instance::unary_encode`](crate::Instance::unary_encode) and
[`Instance::log_encode`](crate::Instance::log_encode) take an explicit
[`ATol`](crate::ATol) and use the SDK's ATol-aware integer-bound
normalization.
[`Instance::log_encode`](crate::Instance::log_encode) rejects integer ranges
that would require more than
[`Instance::MAX_LOG_ENCODING_BITS`](crate::Instance::MAX_LOG_ENCODING_BITS)
binary variables instead of accepting impractically large encoded search spaces.
Both encoders also reject non-point integer ranges outside the unit-spaced
`f64` integer interval, so every accepted encoded integer value remains
distinguishable after adding the encoding offset.
Both methods take an iterator of variable IDs and provide clone-once
transactional bulk encoding; pass a single-element iterator such as `[id]` to
encode exactly one variable. They reject fixed decision-variable IDs before
substitution so fixed values and dependent-variable assignments remain
disjoint.
Fresh decision-variable allocation is now fallible when the `VariableID` space
is exhausted; [`Instance::next_variable_id`](crate::Instance::next_variable_id)
returns an error instead of overflowing.
...
now expose `format_function` for full text rendering and `format_function_with`
for budgeted rendering with [`FunctionFormatOptions`](crate::FunctionFormatOptions).
Both methods validate referenced IDs against the host object before applying
output limits, and [`ParametricInstance`](crate::ParametricInstance) requires
each referenced ID to resolve as exactly one of a decision variable or a
parameter.

The context-free [`Display`](std::fmt::Display) implementation for
[`Function`](crate::Function) still renders raw `x<id>`-style symbols such as
`x1`. Stored nonzero coefficients, including tiny coefficients, are preserved
in that output. The new host-owned APIs use modeling labels, subscripts, and
sorted label parameters from the enclosing instance context.

[`Instance`](crate::Instance) and [`ParametricInstance`](crate::ParametricInstance)
also implement [`Display`](std::fmt::Display) and expose `format_summary` for a
compact whole-instance view. The summary resolves labels for objectives,
