...
solution. A successful call is represented by
[`SolveStatus::Finished`](crate::experiment::SolveStatus::Finished) even when
the SampleSet has no feasible entry.

[`Solve::output`](crate::experiment::Solve::output) and the dynamic-lifetime
counterpart return the new [`SolveOutput`](crate::experiment::SolveOutput)
enum, whose variants contain either [`Solution`](crate::Solution) or
[`SampleSet`](crate::SampleSet). This replaces the Solution-only
`Solve::output_solution` accessor. Newly written Experiment configs use format
version 2; version 1 configs with Solution outputs remain readable.
