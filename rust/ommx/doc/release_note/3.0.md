...
The earlier [`AdditionalCapability`](crate::AdditionalCapability),
...
APIs remain available for explicit special-constraint lowering during the
staged adapter migration. Despite their legacy names, they do not define an
`InstanceClass` or prove membership after lowering.
...
- [`Sampled::append`](crate::Sampled::append) validates existing and
  intra-batch duplicate sample IDs before mutation, so failure leaves the
  collection unchanged. Public, downcastable
  [`SamplesParametersError`](crate::random::SamplesParametersError) signals
  distinguish invalid random sample parameter relations, while sample-ID
  capacity and positive partition generation now handle their full valid
  ranges without integer overflow or strategy panics.
  ([#1107](https://github.com/Jij-Inc/ommx/pull/1107))
- Decision-variable insertion preserves
  [`DecisionVariableError::DuplicateID`](crate::DecisionVariableError::DuplicateID)
  as the table-owned signal, while
  [`AddDecisionVariableError`](crate::AddDecisionVariableError) identifies
  ParametricInstance parameter-ID collisions. Public
  [`ContentFactorError`](crate::ContentFactorError),
  [`OneHotConstraintError`](crate::OneHotConstraintError), and
  [`Sos1ConstraintError`](crate::Sos1ConstraintError) signals distinguish
  model validation failures through the `ommx::Result` error chain.
