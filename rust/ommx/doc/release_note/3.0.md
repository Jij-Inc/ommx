...
values for Experiment refs. Each record includes the Manifest and Config
digests, common status/count fields, Manifest annotations, and the complete
Experiment Config JSON. The listing is backed by digest-addressed SQLite caches
of the original Manifest and Config JSON bytes. Missing rows are backfilled from
the CAS, and cache rows that become unreachable after ref replacement or
