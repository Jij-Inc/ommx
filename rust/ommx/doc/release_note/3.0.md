...
returns the original Manifest JSON byte length plus its config size and unique
layer sizes from the digest-validated SQLite Manifest cache. The
`ommx size <IMAGE_NAMES>...` CLI reports the same logical size for one or more
