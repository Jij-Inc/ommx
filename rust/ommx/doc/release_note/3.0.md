...
`remove_image_ref`. Restore validates the Manifest from the CAS and reports a
conflict instead of replacing a ref that now points to a different digest. The
CLI prints the corresponding `ommx restore-ref` command after each deletion.
