...
[`ExperimentDyn::scoped`](crate::experiment::ExperimentDyn::scoped) and
[`ExperimentDyn::scoped_run`](crate::experiment::ExperimentDyn::scoped_run):
...
interrupted Run and Experiment checkpoint. The
[`ExperimentScope`](crate::experiment::ExperimentScope) passed to an Experiment
callback only creates Runs through `scoped_run`, so an open Run cannot escape
and be omitted from the terminal checkpoint. Rust callers that cannot use a
callback scope can use the existing explicit `commit*` and `finish*` methods.
