...
- **Indicator / one-hot / SOS1** are first-class constraint types alongside
  ...
  `Instance`.
- A **capability model** derives the complete active solver input and compares
  it with coherent native adapter profiles. Explicit preparation can transform
  that input, but the prepared instance must be checked again before solver
  translation.
...
Solver compatibility depends on the complete active input, not only the
presence of indicator / one-hot / SOS1 constraints. For example, adapters can
differ in accepted decision-variable kinds, objective degree, regular
constraint relations and degrees, Indicator body degree, and optimization
sense.

[`Instance::solver_requirements`](crate::Instance::solver_requirements) derives
an [`InstanceRequirements`](crate::InstanceRequirements) snapshot from the
objective and every active constraint family. Only decision variables used by
that solver input participate; fixed, dependent, irrelevant,
removed-constraint-only, and named-function-only variables are excluded.

An [`AdapterCapabilities`](crate::AdapterCapabilities) declaration contains one
or more complete [`CapabilityProfile`](crate::CapabilityProfile) alternatives.
Fields within a profile are conjunctive, while profiles are alternatives. This
can express, for example, continuous QP and MILP support without incorrectly
implying MIQP support. Objective, regular-constraint, and Indicator body limits
use cumulative [`DegreeLimit`](crate::DegreeLimit) values, and relations are
declared independently.

The portable comparison is side-effect free and returns a
[`PortableCompatibilityReport`](crate::PortableCompatibilityReport) with
structured per-profile mismatches and affected variable or constraint IDs.
Profiles describe native direct translator inputs only. Exact reformulation,
relaxation, and heuristic or finite-penalty conversion belong to an explicit
preparation layer, after which requirements must be derived and checked again.
...
are the separate transformation-selection vocabulary. The lowering method takes
the families to lower directly, leaves unselected families unchanged, and
returns only families that were active and actually lowered. These APIs neither
declare native solver support nor establish that a transformed instance is
accepted by a solver; derive fresh solver requirements and check an adapter's
capability profile after preparation.

The legacy `AdditionalCapability`, `Capabilities`, `required_capabilities`,
and `reduce_capabilities` APIs have been removed. This avoids presenting a
transformation selector as native adapter support.

Adapter capabilities are also independent of `ommx.v2` `required_features`:
serialization features determine whether a reader can reconstruct a wire
payload exactly, while capability profiles determine whether a solver
translator can accept the prepared active model.

These three concepts are intentionally distinct: special-constraint kinds
select model transformations, adapter capabilities describe native translator
inputs, and `ommx.v2.Feature` values protect forward-compatible serialization.
