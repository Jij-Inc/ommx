...
[`Function`](crate::Function) is unchanged and still renders raw `x<id>`-style
symbols such as `x1`. The new host-owned APIs use modeling labels, subscripts,
and sorted label parameters from the enclosing instance context.
