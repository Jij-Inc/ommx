...
- Decision variables, parameters, and named functions now follow the same table ownership rule:
  ...
  only `kind` and its exact domain; the [`VariableID`](crate::VariableID),
...
Construct interval-domain `DecisionVariable` rows with
`DecisionVariable::new(kind, bound, atol)` or no-argument factories such as
`DecisionVariable::binary()`. Construct an explicitly enumerated domain with
[`DecisionVariable::new_finite_domain`](crate::DecisionVariable::new_finite_domain),
then insert the row under the desired `VariableID` key. The row owns the
`kind`/domain invariant: interval-domain construction and bound mutation
normalize `bound` with the caller-provided `ATol`, while
[`Kind::FiniteDomain`](crate::Kind::FiniteDomain) stores a validated, non-empty
[`FiniteDomain`](crate::FiniteDomain) as its single source of truth. Its
reported bound is derived from the minimum and maximum enumerated values.
...
[#971](https://github.com/Jij-Inc/ommx/pull/971),
[#976](https://github.com/Jij-Inc/ommx/pull/976),
[#1112](https://github.com/Jij-Inc/ommx/pull/1112).
