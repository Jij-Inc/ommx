...
## 🧱 Experiment recording and persistence

Experiment records now model sampling separately from solving: a
[`Sampling`](crate::experiment::Sampling) stores the complete
[`SampleSet`](crate::SampleSet), and a successful sampling call remains
finished even when no sample is feasible. Solve and Sampling records share the
same adapter-diagnostics payload model.

Run parameters accept non-finite floating-point observations and store the
committed parameter table as MessagePack rather than JSON. Experiment artifacts
use the dedicated `application/org.ommx.v1.experiment` OCI artifact type;
artifacts written by earlier v3 alpha builds with the generic artifact type must
be recreated.

[`AutosavePolicy`](crate::experiment::AutosavePolicy) controls draft checkpoint
frequency without changing explicit failed or interrupted checkpoints.
Experiment and Run attachments support opt-in zstd compression and transparent
decompression, while reader-based and file-based logging stream data into the
Local Registry instead of buffering the complete payload.

Related PRs: [#1033](https://github.com/Jij-Inc/ommx/pull/1033),
...
separate reachability-based operation. Artifact pushes check the destination
repository and skip config or layer blobs that are already present before
...
Experiment ref. Shared blobs are counted once per ref, so these values are not
additive physical Local Registry usage.

Related PRs: [#1026](https://github.com/Jij-Inc/ommx/pull/1026),
