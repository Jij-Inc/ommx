...
[`LocalArtifact::referenced_blob_size`](crate::artifact::LocalArtifact::referenced_blob_size)
returns the total size of unique CAS blobs reachable from one Artifact ref,
including its root manifest, config, layers, and recursive OCI `subject` chain.
The `ommx size <IMAGE_NAMES>...` CLI reports the same logical size for one or
more local Artifact or Experiment refs. Blobs shared by different refs are
counted for each ref, so these values are not additive physical Local Registry
usage.
