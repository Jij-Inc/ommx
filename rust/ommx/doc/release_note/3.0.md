...
- **Indicator / one-hot / SOS1** are first-class constraint types alongside
  ...
  `Instance`.
- An **instance class** describes a set of `Instance` values that an adapter
  can translate directly. Explicit preparation produces another instance,
  whose class membership must be checked again before translation.
...
## 🧮 Instance classes for adapter translation

An adapter's direct-translation boundary is described by an
[`InstanceClass`](crate::InstanceClass): a set of OMMX [`Instance`](crate::Instance)
values defined in terms of the variable kinds referenced by the objective and
active constraints, objective degree, regular- and Indicator-constraint
relations and degree bounds, OneHot / SOS1 presence, and optimization sense.
...
IDs. Preparation or lowering produces another `Instance`, whose membership
must be checked again. Membership does not include adapter-specific
preconditions or successful backend execution.
...
payload exactly, while class membership describes the mathematical structure
accepted by a direct adapter translator.
