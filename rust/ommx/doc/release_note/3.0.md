...
This initial bridge has no payload version negotiation or cross-version
compatibility guarantee. Callers must install a Python OMMX release exposing
the capabilities expected by the bridge; a missing capability raises an
`ImportError` that names it.
