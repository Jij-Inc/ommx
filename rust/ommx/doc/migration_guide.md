...
- [ ] Replace `analyze_decision_variables()` role queries with `Instance::decision_variable_roles()`, `decision_variable_role(id)`, `fixed_decision_variables()`, `dependent_decision_variable_ids()`, or `irrelevant_decision_variable_ids()`; keep `decision_variable_usage()` only when you need reverse expression usage
