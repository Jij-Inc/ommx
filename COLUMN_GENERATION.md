...
この提案の目的は、OMMX における column generation の最小コア API を定め、その前段にある分割情報や高次モデラー連携を、実際に使う側から逆算して設計することである。
...
分割情報、`.dec` loader、高次モデラー連携は、この core API の一段前で RMP 行、初期 column、`PricingOracle` を構成するための builder / helper layer である。
...
重要なのは、入力経路ごとの差を core column generation loop から分離することである。OMMX 固有の分割情報として保存・共有する場合は、`.dec` parser も数式レベルの検知器も、最終的には同じ `Decomposition` へ変換する。一方で、高次モデラー上で直接 RMP / Pricer を構成する場合は、`Decomposition` を経由せずに core API へ接続してもよい。
...
一方、元の OMMX `Instance` から RMP や pricing problem を機械的に生成したい場合には、分割情報が必要になる。分割情報があれば、各 block の local variables、local constraints、master / linking constraints、目的関数と linking 制約への寄与を切り出し、pricing 用の `ParametricInstance` や `PricingOracle` を構成できる。
...
この場合、必ずしも OMMX 固有の `Decomposition` を経由する必要はない。最終的に `ommx-column-generation` が期待する RMP 行、column、`PricingOracle` に落ちればよい。
...
高次モデラー上で RMP / pricing を構成し、`ommx-column-generation` の core API に接続する working example を作る。
...
`ommx-column-generation` は、OMMX における column generation の core API と、その前段にある分割情報・高次モデラー連携を検証するための実験的パッケージとして開始する。
