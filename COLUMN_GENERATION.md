...
- 現状の反復は LP relaxation の root column generation として扱う
...
RMP と pricer は、元の問題をそのまま変数・制約で持つための分割ではなく、「巨大な column 集合を持つ master problem」を暗黙に解くための分割である。

典型的には、Dantzig-Wolfe 分解によって元問題の block feasible set を column として扱う。block を `b in B`、block 内の実行可能解を `x_b in X_b` とすると、元問題は概念的に次のような形を持つ。

```text
min  c0 + sum_b c_b(x_b)

s.t. sum_b A_{i,b}(x_b)  bowtie_i  rhs_i    for linking/master rows i
     x_b in X_b                              for each block b
```

ここで `X_b` は local constraints や local integrality を含んだ block の実行可能集合であり、`A_{i,b}(x_b)` は block 解が linking/master row `i` に与える activity である。この `X_b` の中の実行可能解、あるいはその極点・パターン・経路・スケジュールなどを column として見る。

すべての column を列挙できるなら、解きたい LP relaxation は full master problem として次のように書ける。

```text
min  c0 + sum_{j in J} c_j lambda_j

s.t. sum_{j in J} a_ij lambda_j  bowtie_i  b_i    for master rows i
     lambda_j >= 0
```

ここで `J` は全 column 集合であり、通常は巨大または暗黙的である。column `j` が block 解 `x^j` に対応するなら、`c_j = c(x^j)`、`a_ij = a_i(x^j)` である。block ごとに「1つの block 解を選ぶ」ことを表す convexity constraint が必要な場合は、それも `MasterRow` の一部として持つ。

RMP は、この full master problem の `J` を現在持っている小さい集合 `J'` に制限した問題である。pricer は、まだ RMP に入っていない `J \ J'` の中から、現在の dual に対して reduced cost が負になる column を探す。

したがって、column generation の反復で直接解いているのは「元の flat な MIP/LP」そのものではなく、「元問題または分割情報から導いた full master の LP relaxation」である。元問題との対応は、column が元の block 解やパターンを表し、`Column.payload` に必要な元変数値や block 情報を保持することで復元する。

column generation の中心は、分割情報そのものではなく、この full master / RMP / pricer をつなぐ最小標準形である。
...
現状の column generation loop は LP を想定する。各反復では continuous RMP を解き、その双対値を pricing に渡す。整数解が必要な場合は、生成済み column pool に対して最後に binary RMP を1回解く余地を持たせるが、これは branch-and-price ではなく restricted integer master solve である。
...
- optional な final restricted integer master solve

MVP に含めないもの:

- branch-and-price
- 反復中の integer master solve
- HiGHS model の増分更新
...
- pricing 側の整数性や heuristic 解を `PricingOracle` の責務としてどこまで標準化するか
