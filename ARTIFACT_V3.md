...
**Step F — native archive (tar) reader / writer** (§12.2 行 1 archive 半分、行 4 Archive 残り)。**完了。**

`.ommx` archive は **tar of OCI Image Layout** に過ぎないので、`tar` crate 直叩きの native 実装に置換した。Writer は `LocalArtifact::save` を SQLite から blob を読みながら tar entry を append し、最後に `index.json` (manifest descriptor に `org.opencontainers.image.ref.name` 付き) と `oci-layout` を書く形式に書き直した。Reader 設計は option 2 (`load_archive(file)` を `load_archive(dir)` と同じ "import 経路" に揃え、archive を開いた瞬間に `import_oci_archive` 同等の経路で temp SQLite registry に展開) を採用。`import::archive::import_oci_archive` の OciDir staging は撤去し、tar から直接 `FileBlobStore` + SQLite に publish するようにした。

Step F の landing で ocipkg の `OciArchive` / `OciArchiveBuilder` / `OciDir` / `OciDirBuilder` / `OciArtifact` / `Image` / `ImageBuilder` の使用箇所が全て消え、`Artifact<Base: Image>` の generic 化も削除した。`Artifact<OciArchive>` は `OmmxArchive` (LocalArtifact + tempdir RAII guard) に折り、Python `ArtifactInner::Archive` も同型を保持する。`ArchiveArtifactBuilder` は temp `LocalRegistry` 経由で `LocalArtifactBuilder::build_in_registry` → `LocalArtifact::save` を呼ぶ thin wrapper に書き換え、出力は v3 native save writer に統一された。Python `ArtifactBuilder.new_archive_unnamed` は v3 model (SQLite registry が ref name を要求) と整合しないため削除した。
