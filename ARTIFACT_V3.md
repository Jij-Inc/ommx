...
Build phase では同じ `(space, run_id, media type, name)` に対する Record upsert、同じ `(run_id, parameter_name)` に対する parameter upsert、同じ run attribute key に対する upsert を許容する。
...
Run parameter table と Run attributes は、必ずしも 1 cell / attribute = 1 layer descriptor にならない。run parameter の key-level metadata や実行環境属性は aggregate payload の内部 schema に持たせてよい。Experiment metadata を manifest annotation に物理化する場合も、Record として復元するときは media type + name に写像する。
...
5. Record / Run table / Run attributes の物理化境界
   大きな Record payload は `log_*` 時点で BlobStore に書く前提で、どの media type を即 descriptor 化するか。Run parameter table と Run attributes をどの aggregate JSON / index JSON に materialize するか。
