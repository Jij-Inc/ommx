...
## 5. OCI 実装方針と Local Registry model

§5 (OCI 実装方針) と §6 (Local Registry model) の確定済み仕様は `rust/ommx/doc/artifact_design.md` に移行済み (docs.rs では `ommx::doc::artifact_design` として閲覧可能)。これらは Steps A → H で実装が完了したアーキテクチャであり、本書ではなく API Reference 側を一次資料とする。

旧 §5 / §6 で扱っていた内容:

- OCI implementation policy (external crate scope, public API surface, registry compatibility, OCI Image Layout role, manifest format) → `artifact_design.md` §1
- Local Registry model (role, IndexStore, BlobStore, atomic publish, read/query API, import/export) → `artifact_design.md` §2

未着手領域 (§7 以降) からこれらを参照する場合も `artifact_design.md` を指す。
...
Seal の I/O 段階 — manifest と layer blobs を実際に Local Registry に書き、ref を立てて discoverable にする atomic 動作 — は registry-internal primitive である `publish_artifact_manifest` (`artifact_design.md` §2.4 参照) を呼ぶ。「publish」は OCI Distribution 由来の registry 側の動詞、「commit / build」は SDK ユーザが触る動詞。Git で言えば `git commit` (ユーザ視点) と "object を `.git/objects/` に書いて `refs/heads/<branch>` を進める" (内部実装) の関係に対応する。
...
`subject` の参照先 manifest は OCI Image Manifest であり、`mediaType` は `application/vnd.oci.image.manifest.v1+json` 固定とする。Local Registry が単一 format に閉じている (`artifact_design.md` §1.4) ため、format 跨ぎ dispatch は不要。
...
manifest top-level の `mediaType` field は v2 SDK と同様に出力しない (`artifact_design.md` §1.4)。Content-Type は push 時に transport が個別に付与する。`subject` descriptor の `mediaType` は OCI Image Manifest 固定 (§9.1)。
...
| Image Manifest 一本化方針 (`artifact_design.md` §1.4) と milestone 構成の design merge | #868 |
