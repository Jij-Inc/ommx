...
- Instance、solution、sample set、object、diagnostics は Run に属する Record として記録できる。
...
OMMX では `log_global_*` という命名は採用しない。Experiment object に対する `exp.log_metadata` / `exp.log_object` / `exp.log_environment` は experiment space に、Run object に対する `run.log_*` は run space に記録する。保存先は receiver で決まり、暗黙の context では決めない。ただし `parameter` は Record kind ではなく Run table の列データであり、Experiment には `exp.log_parameter` を持たせない。
...
- diagnostics を DataStore の `media` / `object` / `diagnostic` Record として保存する。
...
DataStore は名前付き Record と Run parameter table の論理 view である。ここで Record と呼ぶものは API / loader から見える単位であり、実装上の blob 所有単位を意味しない。

物理化戦略は kind ごとに変えてよい。`log_instance` / `log_solution` / `log_sample` / 大きな diagnostics payload は `log_*` 時点で BlobStore に保存し、DataStore が digest / size / media type / annotations を持つ descriptor table になる設計が自然である。一方、`run.log_parameter` のような小さな scalar は Record にはせず、Experiment が持つ Run 一覧に付随した表形式データとして扱う。Experiment scope の scalar context は `parameter` ではなく `metadata` として扱い、必要なら manifest annotation または Experiment metadata JSON に物理化する。この節の field は論理モデルであり、最終的な Rust struct の field を固定するものではない。

各 Record は概念的に space、kind、name、content、metadata を持つ。
...
Build phase では同じ `(space, run_id, kind, name)` に対する Record upsert と、同じ `(run_id, parameter_name)` に対する parameter upsert を許容する。Seal / commit phase では最終 DataStore view を復元できる aggregate payload と descriptor set に固定する。View phase の `Artifact` / loaded `Experiment` は immutable とし、追加や更新は新しい builder / Experiment から別 Artifact を作る。
...
Run parameter を key ごとの独立 blob にする必要はない。DataStore の論理 view では `run.log_parameter("seed", 0)` を Run table の cell update として扱い、table extraction や diff は key 単位でできるようにする。一方、Artifact への物理化では小さな scalar をまとめる。
...
`EnvironmentInfo` は DataStore / Artifact の first-class Record として保存する。OTel `Resource` はその投影であり、情報本体ではない。
...
| `ommx.datastore.kind` | `solution`, `instance`, `diagnostic`, ... |
| `ommx.datastore.name` | Record name |
...
Instance / Solution / diagnostics などの payload blob は `log_*` 時点で Local Registry の BlobStore / draft area に逐次保存されていてよい。`commit()` が行うのは、それらの blob と、Run parameter table JSON など commit 時に materialize する payload を含む最終 DataStore view を seal し、復元に必要な descriptor set を immutable manifest として固定することである。
...
| `org.ommx.datastore.kind` | yes | Record kind |
| `org.ommx.datastore.name` | yes | Record name |
| `org.ommx.datastore.scalar_type` | scalar record only | `int`, `float`, `string`, `bool`, `null` 等 |
