...
| Sealed | root manifest blob が BlobStore に保存され、全体を指す root StoredDescriptor が存在する状態。sealed content は digest-addressed immutable data として扱う。 |
...
| Seal | データモデル | unsealed state から root manifest blob を作り、その root `StoredDescriptor` を得る。ref は更新しない。 |
| Publish | Local Registry / IndexStore | sealed root descriptor を ref に対応づけ、Published state にする。payload blob は保存しない。 |
...
| 型 | 役割 |
|---|---|
| `Experiment` | `Arc<LocalRegistry>` + `ExperimentState`。Build phase の mutable session |
...
- `Experiment::log_metadata` / `log_object` / `log_instance` / `log_solution` / `log_sample_set`（experiment space、`&mut self`）
