...
Build phase では同じ `(space, run_id, kind, name)` に対する upsert を許容する。Seal / commit phase では最終 DataStore view を完全な descriptor set として Artifact manifest に固定する。View phase の `Artifact` / loaded `Experiment` は immutable とし、追加や更新は新しい builder / Experiment から別 Artifact を作る。
...
- committed manifest でも、同一 entry blob は CAS で共有できる。
...
各 committed Artifact manifest は、blob bytes の保存タイミングを表すものではない。`layers[]` には、その時点の Experiment view を復元するために必要なすべての descriptor を載せる。

payload blob は `log_*` 時点で Local Registry の BlobStore / draft area に逐次保存されていてよい。`commit()` が行うのは、それらの blob を含む最終 DataStore view を seal し、復元に必要な descriptor set を immutable manifest として固定することである。
