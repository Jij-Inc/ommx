...
- 例外終了時は Run を `status=failed` として close し、例外発生前に記録済みの Attachment / Solve / Run parameter と `failure_reason` を Experiment state に反映する。
- 例外は握りつぶさない。
...
Run status は Experiment config の `runs[].status` に `finished|failed` として保存する。失敗 Run には例外要約として optional な `runs[].failure_reason` を持たせる。elapsed time、実行環境属性、backend solver version などは引き続き未決定であり、Run attributes という aggregate payload は導入していない。
...
通常の成功 commit は config JSON に `status=finished` を持つ Experiment Artifact として requested tag / ref に publish する。例外終了時に作る failed recovery artifact は config JSON に `status=failed` を持ち、manifest annotation に recovery marker を持つ。publish 先は `<registry-id8>.ommx.local/crashed:<local-timestamp>-<nonce>` のような reserved ref とする。`Experiment.load(tag)` の通常 UX は requested tag / ref の成功 Artifact を読む。recovery artifact は `Experiment.load_recovery(...)` / `Experiment.from_recovery_artifact(...)` のような明示 API から扱う。
...
Python context manager 例外時は、成功 Artifact への自動 commit を行わず、その時点で Experiment state に反映済みの Attachment / Run / Solve / parameter を `status=failed` の recovery artifact として materialize する。通常の `Experiment.load(...)` / `Experiment.from_artifact(...)` は成功 Artifact だけを扱い、failed recovery artifact は明示的な recovery API から読み込む。
...
Run status と failure reason は Experiment config の Run entry に保存する。elapsed time、実行環境 OS / package versions / backend solver version などを Artifact data model に持つかどうかは未設計である。
