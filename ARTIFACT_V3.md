...
物理化戦略は entry kind ごとに変えてよい。`log_instance` / `log_solution` / `log_sample` / 大きな diagnostics payload は `log_*` 時点で BlobStore に保存し、DataStore が digest / size / media type / annotations を持つ descriptor table になる設計が自然である。一方、`log_parameter` のような小さな scalar は単独 blob にせず、manifest annotation、Experiment index JSON、または Run ごとの parameter aggregate JSON にまとめて物理化する設計が自然である。この節の field は論理モデルであり、最終的な Rust struct の field を固定するものではない。
...
### 4.2 Scalar / parameter の物理化

MINTO は `parameters` / `meta_data` を aggregate JSON dict として保存していた。OMMX v3 でも、parameter を key ごとの独立 blob にする必要はない。DataStore の論理 view では `run.log_parameter("seed", 0)` を独立 entry として扱い、table extraction や diff は key 単位でできるようにする。一方、Artifact への物理化では小さな scalar をまとめる。
...
この 2 つは論理的には別 entry である。ただし物理的には、例えば run ごとの parameter aggregate JSON、Experiment index JSON、または manifest annotation にまとめて保存してよい。

物理化候補:

| 対象 | 候補 | 備考 |
|---|---|---|
| experiment parameter | manifest annotation または Experiment metadata JSON | 小さい値だけ annotation に載せる。大きい値は JSON layer に逃がす |
...
parameter のように aggregate JSON や manifest annotation に物理化される entry は、必ずしも 1 logical entry = 1 layer descriptor にならない。この annotation schema は typed payload layer や aggregate JSON layer を Experiment DataStore として復元するための最小情報であり、scalar parameter の key-level metadata は aggregate payload の内部 schema に持たせてよい。
