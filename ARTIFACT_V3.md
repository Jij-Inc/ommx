...
保存済み Experiment は read-only view として読み戻す。保存済み Experiment に Run を追加する場合は、既存 Artifact を mutable に戻さず、`fork(...)` で parent Artifact から派生した child Experiment session を作る。
...
Fork の基本操作は実装済みである。`parent()` / `history()` / `diff(other)`、trace diagnostics、failed recovery / autosave metadata は未実装であり、後続節の設計事項として扱う。
...
`Experiment.fork(...)` は実装済みである。保存済み Experiment に Run を追加する操作は、既存 Artifact を mutable に戻すのではなく、loaded Experiment view から新しい unsealed session を作り、parent を `subject` に持つ child Artifact として commit する。`parent()`、`history()`、`diff(other)` は未実装である。
...
| Fork / lineage | 部分実装 | `Experiment.fork(...)` は parent manifest を OCI `subject` に持つ child Experiment を作り、既存 Attachment / Run / Solve / parameter を child state に投影する。new run id は既存最大値 + 1 | `parent()`、`history()`、`diff(other)`、run deletion、lineage retention / GC |
