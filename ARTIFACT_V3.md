...
- parameter は run 比較のための概念であり、Run space にだけ記録する。
...
OMMX では `log_global_*` という命名は採用しない。Experiment object に対する `exp.log_metadata` / `exp.log_object` / `exp.log_environment` は experiment space に、Run object に対する `run.log_*` は run space に記録する。保存先は receiver で決まり、暗黙の context では決めない。ただし `parameter` は Run scope 専用の record kind とし、Experiment には `exp.log_parameter` を持たせない。
...
| 領域 | 正本 | 例 |
|---|---|---|
| 記録データ | DataStore / Artifact | run parameters, metadata, objects, Instance, Solution, SampleSet, EnvironmentInfo |
...
物理化戦略は entry kind ごとに変えてよい。`log_instance` / `log_solution` / `log_sample` / 大きな diagnostics payload は `log_*` 時点で BlobStore に保存し、DataStore が digest / size / media type / annotations を持つ descriptor table になる設計が自然である。一方、`run.log_parameter` のような小さな scalar は単独 blob にせず、Experiment index JSON または Run ごとの parameter aggregate JSON にまとめて物理化する設計が自然である。Experiment scope の scalar context は `parameter` ではなく `metadata` として扱い、必要なら manifest annotation または Experiment metadata JSON に物理化する。この節の field は論理モデルであり、最終的な Rust struct の field を固定するものではない。
...
| `kind` | `parameter`, `metadata`, `object`, `instance`, `solution`, `sampleset`, `environment`, `diagnostic`, `media`。`parameter` は run scope only |
| `name` | space + kind 内の user-facing key |
| `content` | scalar value、serialized bytes、または blob descriptor への参照 |
| `media_type` | Artifact layer media type。単独 layer にならない entry では aggregate payload 内の type 情報でもよい |
| `annotations` | Artifact descriptor annotations に投影される metadata |

Build phase では同じ `(space, run_id, kind, name)` に対する upsert を許容する。Seal / commit phase では最終 DataStore view を復元できる aggregate payload と descriptor set に固定する。View phase の `Artifact` / loaded `Experiment` は immutable とし、追加や更新は新しい builder / Experiment から別 Artifact を作る。

### 4.2 Run parameter の物理化

MINTO は `parameters` / `meta_data` を aggregate JSON dict として保存していた。OMMX v3 では、`parameter` を Run scope 専用の概念として扱う。Experiment 全体に属する dataset、source problem、baseline、analysis context は `metadata` または `object` であり、`parameter` ではない。

Run parameter を key ごとの独立 blob にする必要はない。DataStore の論理 view では `run.log_parameter("seed", 0)` を独立 entry として扱い、table extraction や diff は key 単位でできるようにする。一方、Artifact への物理化では小さな scalar をまとめる。
...
この 2 つは論理的には別 entry である。ただし物理的には、例えば run ごとの parameter aggregate JSON または Experiment index JSON にまとめて保存してよい。
...
| adapter kwargs | run parameter として aggregate JSON に含める | scalar kwargs のみ。Instance などは別 entry |
| table index | Experiment index JSON | table 再構成を速くするための derived payload として持てる |

この方針では、`parameter` は API / analysis 上の最小単位であり、Artifact layer の最小単位ではない。`Instance` や `Solution` のような大きな typed payload は即 blob 化しやすいが、scalar parameter は commit 時に aggregate payload として materialize する方が manifest / blob 数を抑えられる。
...
| Kind | Payload | 備考 |
|---|---|---|
| `parameter` | scalar JSON | run scope only。int / float / str / bool / null 相当。numpy scalar は Python 側で正規化する |
...
Trace layer は DataStore の代替ではない。parameter / solution / sample set / environment の本体は DataStore の物理化戦略に従って保存し、trace layer は実行時系列と record reference を保存する。
...
Instance / Solution / diagnostics などの payload blob は `log_*` 時点で Local Registry の BlobStore / draft area に逐次保存されていてよい。`commit()` が行うのは、それらの blob と、parameter aggregate JSON など commit 時に materialize する payload を含む最終 DataStore view を seal し、復元に必要な descriptor set を immutable manifest として固定することである。
...
parameter のように aggregate JSON に物理化される entry は、必ずしも 1 logical entry = 1 layer descriptor にならない。この annotation schema は typed payload layer や aggregate JSON layer を Experiment DataStore として復元するための最小情報であり、run parameter の key-level metadata は aggregate payload の内部 schema に持たせてよい。Experiment metadata を manifest annotation に物理化する場合も、`parameter` ではなく `metadata` として復元する。
...
- 同一 `(space, run_id, kind, name)` が複数 layer に現れた場合、manifest order の最後を採用するか error にするかを決める必要がある。提案としては committed manifest では重複を禁止し、loader は error にする。
...
同じ仕組みで、派生した Experiment version から run を削除する操作も表現できる。削除は既存 blob を消す操作ではなく、新しい manifest からその run に対応する descriptor、aggregate parameter entry、index record を省く操作である。元 Artifact は immutable な parent として残り、削除された run の blob は parent からは引き続き参照され得る。物理的な blob 削除は GC の到達可能性解析と retention policy に委ねる。
...
GC は data model を変えない。完全な descriptor set を持つ manifest、digest primary、single-parent lineage、DataStore entry model とは独立した maintenance operation とする。
