...
MINTO の `Experiment` context manager は `auto_saving` が有効なら保存 directory へ flush していたが、archive commit までは行っていなかった。OMMX では常時 draft autosave を持つため、`with Experiment(...)` の意味を「保存 directory への flush」ではなく「Experiment session の正常終了と Artifact commit」に寄せる。
...
| 相 | 性質 | API |
|---|---|---|
| Build | mutable。常に durable draft を持つ | `Experiment`, `Run`, builder |
...
Run close は Artifact commit ではない。Run close は Experiment state を更新し、常時 draft autosave に反映される。

### 3.4 常時 Draft Autosave
...
`log_*` 時点で大きな Instance / solver log / diagnostics payload を draft storage に spill してよいが、それは未 publish の draft data である。public ref に到達可能な Artifact manifest は commit まで作らない。
...
新しい Run は既存 `run_id` と衝突しない id を割り当てる。正常終了時の自動 commit では parent を `subject` に持つ child manifest を作る。例外終了時は child Artifact を commit しない。draft recovery は forked session の draft を復元するためのものであり、parent Artifact を変更するものではない。
...
2. Context manager の詳細 semantics
   `with Experiment(...)` は正常終了時に自動 commit する方針とする。残る論点は、commit 済み session への追加を禁止する API、`exp.artifact` の availability、例外終了時の draft recovery UX、tag が省略された場合の扱いである。
...
- `with Experiment(...)` の正常終了 auto commit、例外終了 draft retention、commit 後 mutation 禁止を実装する。
