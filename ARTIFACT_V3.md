...
Trace ID は OMMX が任意に採番するのではなく、OTel の root span 作成時に発行される。OMMX は以下のルールで span を開始し、生成された Artifact の manifest annotations に build span の `trace_id` / `span_id` を記録する (キー命名は 6 章 Q3)。
...
- `Artifact.load*` / `push`:
  ...
  - 読み込んだ artifact 内の build-time trace は OTel Link または attribute として参照する
- 派生 artifact:
...
- canonical format は OTLP。Chrome Trace Event Format は表示・Notebook 用に読み出し時に変換する
- Phase 1 では span / span event を対象とし、OTel Logs / Metrics の埋め込みは Phase 2 以降に回す
- Experiment / Run API 経由で生成される Artifact は trace layer を default-on にする
...
- 対象 trace_id だけを購読する scoped `SpanProcessor` を使う
...
4.3.1 節。Artifact の manifest annotations に build span の `trace_id` / `span_id` を残すことは決定済み。残る論点はキー命名規約 (`org.ommx.trace.*` 等) と、再ロード時の取り扱い (新しいトレースの `links` として張るか、単なる属性として表示するか)。

### Q4. トレース本体の Artifact 内埋め込み

4.3.2 節。「埋め込む」自体は決定済み (Cloud Run / バッチ系での要件、1 章方針)。さらに以下を固定する:

- canonical format は OTLP。Chrome Trace Event Format は表示用の派生形式
...
残る論点は media type 命名 (`application/vnd.opentelemetry.trace.otlp+json` / 独自 `application/org.ommx.trace+json` 等) と、OTLP を Protobuf / JSON のどちらで保存するか。
