...
- `with Experiment(...)` は正常終了時に immutable Artifact version を自動 commit する。
...
- 実験途中で process が落ちても、常時有効な draft autosave から既に記録したデータを可能な範囲で復元できる。
...
Record kind は space に依存しない。`metadata`, `object`, `instance`, `solution`, `sampleset`, `diagnostic`, `media` は Experiment space / Run space のどちらにも置ける。例えば全 run で共有する source Instance は Experiment space に、実際に各 run で解いた candidate Instance は Run space に置く。

実行環境は Record ではなく Run attribute とする。保存済み Experiment から後で新しい Run を追加できる以上、実行環境は Experiment 全体の不変属性ではない。Experiment を作成した環境や import / load した環境を残したい場合は、実行環境ではなく metadata / provenance として扱う。
...
Experiment state は名前付き Record set、Run parameter table、Run attributes からなる。ここで Record と呼ぶものは API / loader から見える単位であり、実装上の blob 所有単位を意味しない。

物理化戦略は kind ごとに変えてよい。`log_instance` / `log_solution` / `log_sample` / 大きな diagnostics payload は `log_*` 時点で draft storage に保存し、Record が digest / size / media type / annotations を持つ descriptor reference になる設計が自然である。一方、`run.log_parameter` のような小さな scalar は Record にはせず、Experiment が持つ Run 一覧に付随した表形式データとして扱う。Experiment scope の scalar context は `parameter` ではなく `metadata` として扱い、必要なら manifest annotation または Experiment metadata JSON に物理化する。この節の field は論理モデルであり、最終的な Rust struct の field を固定するものではない。

各 Record は概念的に space、kind、name、content、metadata を持つ。Record kind は space によって制限しない。`parameter` と Run 実行環境属性は Record ではないため、この規則の対象外である。
...
Build phase では同じ `(space, run_id, kind, name)` に対する Record upsert、同じ `(run_id, parameter_name)` に対する parameter upsert、同じ run attribute key に対する upsert を許容する。Seal / commit phase では最終 Experiment state を復元できる aggregate payload と descriptor set に固定する。View phase の `Artifact` / loaded `Experiment` は immutable とし、追加や更新は新しい builder / Experiment から別 Artifact を作る。
...
実行環境は Run attribute として保存する。OTel `Resource` はその投影であり、情報本体ではない。
...
| 相 | 性質 | API |
|---|---|---|
| Build | メモリ上では mutable。常に durable draft を持つ | `Experiment`, `Run`, `ArtifactBuilder` |
...
- `with Experiment(...)` は mutable session の lifetime を表し、正常終了時に自動 commit する。
- `with exp.fork(...)` 相当の re-enter は、parent Experiment を mutable に戻すのではなく、parent Artifact を lineage に持つ派生 session を開く。
- block 内で明示 `commit()` 済みの場合、`__exit__` の commit は no-op にする。
- 例外終了時は commit せず、draft を recovery 用に残す。
...
- 一方、長時間実験では途中結果を durable にしたいので、public commit とは別の draft storage を常に持つ。

### 5.3 常時 Draft Autosave

`autosave` は user-facing な有効 / 無効 option にはしない。`Experiment` は常に draft autosave を持ち、`autosave=False` はサポートしない。
...
exp = Experiment.load("scip_reblock115:v1")
...
forked session では parent に含まれる既存 Record、Run parameter table、Run attributes は読み取り可能な初期 state として見える。ただし parent Experiment / Artifact 自体は immutable であり、変更は child Artifact の manifest にだけ反映される。新しい Run は既存 `run_id` と衝突しない id を割り当て、正常終了時の自動 commit で parent を `subject` に持つ child manifest を作る。

例外終了時は child Artifact を commit しない。draft recovery は派生 session の draft を復元するためのものであり、parent Artifact を変更するものではない。
...
Artifact lineage は OCI v1.1 `subject` で表す。v3 初期は各 Artifact が 0/1 個の parent を持つ single-parent history のみを扱う。
...
2. Context manager の詳細 semantics
   `with Experiment(...)` は正常終了時に自動 commit する方針とする。残る論点は、commit 済み session への追加を禁止する API、`exp.artifact` の availability、例外終了時の draft recovery UX、re-enter 時の tag / parent metadata の扱いである。

3. Autosave storage
   Local Registry draft として実装するか、SDK-owned working directory として実装するか。

4. Record / Run table / Run attributes の物理化境界
   大きな Record payload は `log_*` 時点で draft storage に spill する前提で、どの kind を即 descriptor 化するか。Run parameter table と Run attributes をどの aggregate JSON / index JSON に materialize するか。
