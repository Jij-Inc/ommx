...
§5 (OCI 実装方針) と §6 (Local Registry model) は実装完了済みで、本書ではなく以下を一次資料とする:

- **OMMX Artifact format spec** (manifest 形式、OCI Image Layout boundary、registry compatibility) → `rust/ommx/doc/artifact_design.md` (docs.rs: `ommx::doc::artifact_design`)
- **Local Registry の内部実装** (SQLite IndexStore + filesystem CAS BlobStore、atomic publish、lazy auto-migration、import / export) → `rust/ommx/src/artifact/local_registry/*.rs` の module rustdoc (docs.rs: `ommx::artifact::local_registry`)

未着手領域 (§7 以降) からこれらを参照する場合も上記を指す。
...
`subject` の参照先 manifest は OCI Image Manifest であり、`mediaType` は `application/vnd.oci.image.manifest.v1+json` 固定とする。Local Registry が単一 format に閉じている (`artifact_design.md` §1 (Manifest format)) ため、format 跨ぎ dispatch は不要。
...
manifest top-level の `mediaType` field は v2 SDK と同様に出力しない (`artifact_design.md` §1 (Manifest format))。Content-Type は push 時に transport が個別に付与する。`subject` descriptor の `mediaType` は OCI Image Manifest 固定 (§9.1)。
...
| Image Manifest 一本化方針 (`artifact_design.md` §1 (Manifest format)) と milestone 構成の design merge | #868 |
