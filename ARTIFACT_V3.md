...
- run ごとに変わる formulation / strategy などの parameter を Run table の列として記録し、後から比較できる。
- Instance、Solution、SampleSet、JSON、diagnostics、user-defined media type は Experiment / Run のどちらにも Attachment として記録できる。
- parameter は Attachment ではなく、Experiment が持つ Run 一覧に付随する表形式データとして扱う。
- solver adapter に実際に渡した kwargs は Run parameter ではなく Solve parameter として記録する。
...
Run は Artifact sub manifest ではなく、Experiment config 内の `run_id` で束ねられる logical entity とする。Run の保存実体は、Run parameter table の row、Run-scoped Attachment への `LayerRef`、Run 内の Solve list、trace layer 内の `ommx.run` span である。
...
- adapter name / backend solver name / scalar kwargs は Run parameter ではなく Solve parameters として記録する。
...
`run.log_solve(...)` は暫定実装があるが、現行実装は input / output を Run-scoped Record として保存し、adapter kwargs を Run parameter table に入れる形であり、本節の目標モデルとは異なる。`run.log_sample(...)`、diagnostics sink protocol、adapter diagnostics Attachment は未実装である。

残作業:

- Experiment config に Run 内の Solve list を導入する。
- Solve input / output を `LayerRef` で参照し、常に caller が渡した Instance と adapter が返した Solution を保存する。
- adapter 名、backend solver 名、adapter kwargs を Run parameter ではなく Solve parameter として保存する。
- 1 Run 内で複数 Solve を自然に並べられる `solve_id` 採番と API を決める。
...
| Experiment config JSON | 実装済み（現行） | 現行 wire shape は `status`、Experiment-space Record descriptors、Run list、Run-scoped Record descriptors、Run parameter table descriptor を保存する | Solve list、Attachment、`LayerRef` を持つ target schema への移行、schema evolution policy |
...
| Rust `SealedExperiment` view | 実装済み（現行） | Config から Experiment-space Records、Run list、Run-scoped Records、Run parameter cells を復元する | Solve / Attachment view、higher-level table / summary views |
...
| Adapter `log_solve` / `log_sample` | 部分実装 | `Run.log_solve` は Python で暫定実装済み。現行は input / output を Run-scoped Record、kwargs を Run parameter として保存する | Solve list model、kwargs の Solve parameter 化、input / output の `LayerRef` 化、`log_sample`、API 名の整理 |
