...
- `Run.log_solve` が 1 Run 内の複数 Solve を `solve_id` 順に保存し、kwargs を Solve parameters として復元すること。
...
`run.log_solve(...)` は、Run 内の Solve list、Solve input / output の `LayerRef`、Solve parameters への kwargs 保存まで実装済みである。`run.log_sample(...)`、diagnostics sink protocol、adapter diagnostics Attachment は未実装である。
...
| Adapter `log_solve` / `log_sample` | 部分実装 | `Run.log_solve` は Python で実装済み。adapter 名と kwargs は Solve parameter、input / output は Solve entry の `LayerRef` として保存する | `log_sample`、backend solver 名 / status / elapsed time、API 名の整理 |
