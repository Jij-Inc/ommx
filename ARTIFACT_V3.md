...
- 正常終了時は自動 commit する。
- 例外終了時は成功 commit を行わず、その時点で closed Run として Experiment state に反映済みの情報を `status=failed` の failed recovery Artifact として reserved ref に publish する。
- failed recovery Artifact は成功 ref を進めない。process kill など Python context manager が走らない場合の autosave metadata からの復元は未実装である。
...
- 例外終了時は Run を `status=failed` として close し、例外発生前に記録済みの Attachment / Solve / Run parameter を Experiment state に反映する。
- 例外は握りつぶさない。
...
新しい Run は既存 `run_id` と衝突しない id を割り当てる。正常終了時の自動 commit では parent を `subject` に持つ child manifest を作る。例外終了時は child の成功 Artifact を commit せず、復元可能な範囲を failed recovery Artifact として reserved ref に publish する。autosave metadata による recovery は forked session を復元するためのものであり、parent Artifact を変更するものではない。
...
Run status は Experiment config の `runs[].status` に `finished|failed` として保存する。例外要約は、実験に使ったコード、入力、実行環境と結びつかない限り原因究明に足る情報になりにくいため、Run metadata としては保存しない。elapsed time、実行環境属性、backend solver version などは引き続き未決定であり、Run attributes という aggregate payload は導入していない。
...
通常の成功 commit は config JSON に `status=finished` を持つ Experiment Artifact として requested tag / ref に publish する。例外終了時に作る failed recovery artifact は config JSON に `status=failed` を持ち、manifest annotation に recovery marker と original requested image name を持つ。publish 先は `<registry-id8>.ommx.local/crashed:<local-timestamp>-<nonce>` のような reserved ref とする。`Experiment.load(tag)` の通常 UX は requested tag / ref の成功 Artifact を読む。`Experiment.load_recovery(crashed_ref)` / `Experiment.from_recovery_artifact(...)` は recovery Artifact を parent にし、original requested image name で fork した unsealed Experiment を返す。
...
Python context manager 例外時は、成功 Artifact への自動 commit を行わず、その時点で Experiment state に反映済みの Attachment / Run / Solve / parameter を `status=failed` の recovery artifact として materialize する。通常の `Experiment.load(...)` / `Experiment.from_artifact(...)` は成功 Artifact だけを扱う。failed recovery artifact は明示的な recovery API から読み込み、API は recovery Artifact そのものの read-only view ではなく、元の requested image name で fork 済みの unsealed Experiment を返す。

process crash など Python context manager が走らない場合に、どの Attachment / Run / Solve / parameter がどの blob に対応していたかを復元する autosave metadata は未設計である。

残作業:

- uncommitted Experiment state の autosave metadata format。
- process crash 後の autosave metadata から recovery artifact を再構成する command / inspector の API。
...
| 項目 | 残作業 |
|---|---|
| Failed recovery / autosave metadata | process crash 向け metadata format、recovery command、retention policy |
| Recovery artifact | recovery artifact inspector、crash 後の autosave metadata からの再構成 |
