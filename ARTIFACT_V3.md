...
2. 実験管理 (Experiment / Run / DataStore) は OMMX が直接提供する。`minto` は OMMX を利用するドメイン固有ライブラリの位置付けに移る。
3. observability は `ommx.tracing` (OTel) に一元化される。`MintoLogger` 相当の階層出力は OTel span のレンダラとして実装され、独立ログ系統は持たない。
...
- ocipkg 開発当時は Rust 製の OCI spec ライブラリが未整備で自前実装が多い。現在は `oci-spec` 等の整備が進んだので、外部ライブラリを使える箇所は使うのが筋
...
- `rust/ommx/src/artifact/` 内の OCI 取り扱いコードの自前化、`Cargo.toml` から `ocipkg` を削除
- minto の汎用機能 (DataStore / ExperimentDataSpace / table / EnvironmentInfo / Logger) を **再設計しつつ** OMMX に取り込む
...
- OMMX 自身の v2 → v3 の Python API 互換性は別論点 (6 章 Q2 参照)
...
- **Push / Pull**: remote 経由のネットワーク I/O は OTel semantic conventions の `http.*` 属性で計装する
...
選択は 6 章 Q4 を参照。なお 4.5 節で MintoLogger を分解する場合、リアルタイム描画は streaming SpanProcessor として実装される (これが分類 A/B のコンソール出力の唯一の経路)。
...
| 複数 experiment の統合 | 単一 `subject` では表現不能、複数 parent 規約は 6 章 Q8 |

未解決の設計点は 6 章 Q7〜Q11 (commit 粒度、parent 数、tag 可視性、GC、diff API)。
...
最終形が remote registry への push/pull (OCI Distribution Spec) を含むか、含むなら実装は:

- (a) 既存クレート (`oci-distribution`, `oci-client` 等) を採用 — 1 章の方針 (自前実装は最小限) と整合、**基本線**
- (b) OCI Distribution Spec を `reqwest` ベースで自前実装
- (c) archive / dir のみ対応し、remote は最終形のスコープ外

(a) と (c) のどちらを取るか (= 最終形に remote 機能を含めるか) と、(a) を取る場合の依存クレート選定が論点。

### Q2. 後方互換性ポリシー

- 既存 Python API (`Artifact.load_archive` / `ArtifactBuilder.new_archive` 等) のシグネチャを v3 で維持するか
- minto 取り込みに伴う `ommx.artifact` の API 拡張は破壊的変更とせず additive にできるか
...
### Q4. Logger / OTel 統合のリアルタイム性

4.6 節の選択。最終形がリアルタイムレンダラを提供するか、ポストホックに統一するか、両方提供か。

### Q5. Artifact annotation へのトレース情報埋め込み

4.3 節の通り、Artifact の manifest annotations に build 時の `trace_id` / `span_id` を残すか。残す場合のキー命名規約 (`org.ommx.trace.*` 等) と、再ロード時の取り扱い (新しいトレースの `links` として張る? 単なる属性として表示?)。

### Q6. OTel semantic conventions の採用範囲
...
- **OTel TracerProvider の二重登録**: minto も独自に provider を立てている場合、取り込み後は `ommx.tracing` の lazy 初期化と衝突しないか要検証 (OTel は provider 差し替えを拒否するため、テスト初期化順が壊れやすい — `test_tracing_magic.py` の冒頭参照)
