...
EnvironmentInfo は Record ではなく Run attribute とする。保存済み Experiment から後で新しい Run を追加できる以上、実行環境は Experiment 全体の不変属性ではない。Experiment を作成した環境や import / load した環境を残したい場合は、実行環境ではなく metadata / provenance として扱う。
...
各 Record は概念的に space、kind、name、content、metadata を持つ。Record kind は space によって制限しない。`parameter` と `EnvironmentInfo` は Record ではないため、この規則の対象外である。
...
Run parameter table は Record とは別に、`run_id` と parameter name を key にした scalar table として持つ。Run attributes は `run_id` を key にした structured attributes であり、初期設計では `EnvironmentInfo` を含む。
...
`EnvironmentInfo` は Run attribute として保存する。OTel `Resource` はその投影であり、情報本体ではない。
...
Instance / Solution / diagnostics などの payload blob は `log_*` 時点で Local Registry の BlobStore / draft area に逐次保存されていてよい。`commit()` が行うのは、それらの blob と、Run parameter table / Run attributes JSON など commit 時に materialize する payload を含む最終 Experiment state を seal し、復元に必要な descriptor set を immutable manifest として固定することである。
...
Run parameter table と Run attributes は、必ずしも 1 cell / attribute = 1 layer descriptor にならない。run parameter の key-level metadata や `EnvironmentInfo` は aggregate payload の内部 schema に持たせてよい。Experiment metadata を manifest annotation に物理化する場合も、`parameter` ではなく `metadata` として復元する。
...
4. Record / Run table / Run attributes の物理化境界
   Record payload を `log_*` 時点で BlobStore に保存するか、commit まで payload を保持して Seal phase で descriptor 化するか。Run parameter table と Run attributes をどの aggregate JSON / index JSON に materialize するか。
...
- `EnvironmentInfo` を Run attribute として実装する。
