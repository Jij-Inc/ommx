...
5. Record / Run parameter table の物理化境界
   大きな Record payload は `log_*` 時点で BlobStore に書く前提で、どの media type を即 descriptor 化するか。Run parameter table をどの aggregate JSON / index JSON に materialize するか。
...
1. Run parameter table JSON / Experiment index JSON / OCI empty config を Store し、それぞれ `StoredDescriptor` を得る。
