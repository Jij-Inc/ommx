...
- **OMMX は global `TracerProvider` を暗黙に設定しない**: import / `Experiment` 開始 / `ArtifactBuilder.build()` で `trace.set_tracer_provider()` を勝手に呼ばない。ユーザまたは実行環境が設定した provider に span / event を流し、未設定時は no-op または明示的な setup error とする
...
- Experiment / Run API 経由で生成される Artifact は trace layer を default-on にする。ただし OMMX は global `TracerProvider` を暗黙設定しないため、span を収集できる provider / collector が未設定の場合は trace layer を生成しないか、明示的な setup error とする
- 低レベル `ArtifactBuilder` は `with_trace()` 相当の明示 opt-in にする
...
未解決の設計点は 6 章 Q9〜Q10 (GC、diff API)。
...
- Experiment / Run API 経由の Artifact は trace layer default-on
- 低レベル `ArtifactBuilder` は `with_trace()` 相当の明示 opt-in
- OMMX は global `TracerProvider` を暗黙設定しない。trace layer default-on は、span を収集できる provider / collector が存在する場合の既定動作

残る論点は、未設定 provider のときに trace layer を静かに省略するか、明示的な setup error にするかの UX だけ。
...
- **OTel TracerProvider の所有権**: OMMX は `trace.set_tracer_provider()` を暗黙に呼ばない方針にする。現行 `ommx.tracing` の lazy 初期化が provider を install する挙動は v3 で見直しが必要。未設定 provider のときの trace layer UX (省略 / setup error) は実装時に詰める
