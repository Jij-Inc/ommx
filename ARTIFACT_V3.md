...
この model は MLflow の Experiment / Run 階層と似ているが、OMMX では Experiment を単なる run grouping にはしない。数理最適化の実験では、同じ source problem から定式化を変える、一部の制約を無効化する、relaxation を変える、solver parameters を変える、といった複数 run を比較することが多い。したがって run-specific Instance は自然に Run space に記録される。Experiment space は全 run に共通する source data、baseline、dataset identity、environment、analysis context を持ち、共有入力、複数試行、比較 table、共有 Artifact を 1 つの experiment bundle として扱うために first-class DataStore を持つ。
...
- `Instance` kwargs は、その run で実際に解いた candidate Instance として記録できる。全 run で本当に共有される巨大な Instance だけは、experiment space entry への explicit reference を使えるようにする。
