...
local の 1 つ以上の Artifact または Experiment image ref が参照する論理サイズは、
{command}`ommx size` で確認できます。
...
| anonymous Artifact / Experiment refs | ref が存在する間は Yes | `ommx prune-anonymous` は anonymous Artifact refs を削除します。`--experiments` を付けると anonymous Experiments も対象になります。その後の `ommx gc` で、到達不能になった blob を回収できます。 |
...
anonymous Experiment も対象にする場合は明示的に指定し、`--older-than` で
経過時間に基づく retention を設定できます。
...
一時的な Artifact build や名前なし archive import から anonymous Artifact ref が残っている場合は、先に {command}`ommx prune-anonymous` を実行します。anonymous Experiment session も対象にするには `--experiments` を追加します。この command は該当する SQLite refs だけを削除し、blob は unlink しません。その blob は、他の ref から到達できなければ {command}`ommx gc` で回収可能になります。candidate 選択後に ref が置き換えられた場合は stale candidate として削除しないよう、prune は compare-and-delete で実行されます。
