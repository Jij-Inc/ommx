...
以下は PySCIPOpt Adapter の例です。PySCIPOpt Adapter は、SCIP が progress event を
出した場合に {class}`~ommx_pyscipopt_adapter.SCIPProgressReport` を記録し、その後に
...
監視し、少なくとも 1 つの event が観測された場合に
{class}`~ommx_pyscipopt_adapter.SCIPProgressReport` を 1 つ記録します。

report は {class}`~ommx_pyscipopt_adapter.SCIPProgressSnapshot` の `snapshots` tuple を
持ちます。各 snapshot は SCIP event callback の中で見えている model state です。
