...
PySCIPOpt Adapter は `solve` の内部で OneHot を明示的に lowering し、通常の等式制約 $x_0 + x_1 + x_2 - 1 = 0$ に変換してから SCIP に渡します。
