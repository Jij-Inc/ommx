...
PySCIPOpt Adapter は現在、legacy な lowering selector で Indicator と SOS1 を維持し、SCIP の `addConsIndicator` / `addConsSOS1` にそのまま渡します（等式 Indicator は上下2本の不等式 Indicator に分解されます）。OneHot は維持されないため、SCIP に渡す前に通常の等式制約へ lowering されます。この selector は `INPUT_CLASS` の membership や Adapter applicability とは別です。詳しくは [Adapter の入力 class と legacy な特殊制約 lowering](./capability_model.md) を参照してください。
...
PySCIPOpt Adapter は Indicator 制約を legacy lowering の対象外として維持し、そのまま SCIP に渡します。
...
PySCIPOpt Adapter は OneHot を legacy lowering で維持しないため、`solve` の内部で通常の等式制約 $x_0 + x_1 + x_2 - 1 = 0$ に変換してから SCIP に渡します。
...
PySCIPOpt Adapter は SOS1 制約を legacy lowering の対象外として維持し、そのまま SCIP に渡します。
...
ただし、特殊制約型を通常制約に変換する（[legacy な特殊制約 lowering](./capability_model.md) 参照）と、新たに生成される通常制約は **`Constraint` 側の ID 空間**から割り当てられます。変換後に衝突する可能性があるのは通常制約の ID のみです。
...
OneHot / SOS1 については、`removed_one_hot_constraints` / `removed_sos1_constraints` への移動は [legacy な特殊制約 lowering](./capability_model.md) で扱う変換 API によって行われます。
