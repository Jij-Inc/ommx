...
PySCIPOpt Adapter の native capability profile は Indicator と SOS1 を受理し、SCIP の `addConsIndicator` / `addConsSOS1` にそのまま渡します（等式 Indicator は上下2本の不等式 Indicator に分解されます）。OneHot は直接は受理しません。OneHot を通常の等式制約へ lower する処理は、Adapter Capability ではなく明示的な preparation step です。Native profile、preparation、serialization feature の違いは [Adapter Capability モデルと明示的な preparation](./capability_model.md) を参照してください。
...
PySCIPOpt Adapter の native profile はこの線形 Indicator 制約を受理するので、そのまま求解できます。
...
PySCIPOpt Adapter は direct translator 入力として OneHot を受理しません。Compatibility check は `instance_oh` を変更せず、その不一致を報告します。
...
Lowering する意図がある場合は OneHot を明示的に選択し、prepared model を再検査します。この exact lowering で通常の等式制約 $x_0 + x_1 + x_2 - 1 = 0$ が作られます。
...
`lower_special_constraints` は選択した preparation workspace を in-place に変更します。後続の compatibility check と `solve` はこの lowering を行いません。明示的な変換の記録は `removed_one_hot_constraints` に残ります。
...
PySCIPOpt Adapter の native profile は SOS1 制約を受理するので、そのまま求解できます。
...
ただし、特殊制約型を通常制約に変換する（[Adapter Capability モデルと明示的な preparation](./capability_model.md) 参照）と、新たに生成される通常制約は **`Constraint` 側の ID 空間**から割り当てられます。変換後に衝突する可能性があるのは通常制約の ID のみです。
...
OneHot / SOS1 については、`removed_one_hot_constraints` / `removed_sos1_constraints` への移動は [Adapter Capability モデルと明示的な preparation](./capability_model.md) で扱う明示的な lowering および変換 API によって行われます。
