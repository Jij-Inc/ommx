...
v3 では Artifact の保存先を SQLite-based Local Registry に統一しました。Artifact の blob は content-addressed storage に保存され、image name から manifest への参照は SQLite で管理されます。これにより、ローカルにある Artifact は image name で一貫して参照できるようになります。

この変更に合わせて、`.ommx` アーカイブは Local Registry へ import / export するための交換用フォーマットとして整理しました。{class}`~ommx.artifact.ArtifactDraft` と {class}`~ommx.artifact.Artifact` に破壊的変更が入っています:
...
- `ArtifactBuilder.new_archive_unnamed` → {func}`ArtifactDraft.new_anonymous <ommx.artifact.ArtifactDraft.new_anonymous>` + `Artifact.save(path)`。匿名アーティファクトは `None` の代わりに `<registry-id8>.ommx.local/anonymous:<timestamp>-<nonce>` 形式の自動生成された image_name を持ちます。
