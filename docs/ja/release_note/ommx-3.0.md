...
### 🆕 検証付き one-hot promotion ([#1050](https://github.com/Jij-Inc/ommx/pull/1050))

{class}`~ommx.OneHotPromotionWitness` と新しい
...
非変更の preview には {meth}`~ommx.Instance.check_promotion_witness`、
all-or-nothing の一括昇格には
{meth}`~ommx.Instance.promote_with_witnesses` を使います。元の regular
