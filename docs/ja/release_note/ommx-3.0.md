...
### 🆕 構造制約で DecisionVariable を受け取る API ([#1078](https://github.com/Jij-Inc/ommx/pull/1078))

{class}`~ommx.OneHotConstraint` と {class}`~ommx.Sos1Constraint` の
`variables` 引数は、生の変数 ID ではなく {class}`~ommx.DecisionVariable` または
{class}`~ommx.AttachedDecisionVariable` の object を受け取るようになりました。
制約は引き続き OMMX の変数 ID を内部に保存し、`variables` property も従来どおり
それらの ID を返します。
