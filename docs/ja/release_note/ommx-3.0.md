...
### 🆕 HiGHS / Python-MIP Adapter の native capability 宣言 ([#1085](https://github.com/Jij-Inc/ommx/pull/1085))

`OMMXHighsAdapter` と `OMMXPythonMIPAdapter` は、native translator が直接
扱える入力全体を宣言し、変換前に検査するようになりました。対象は Binary、Integer、
Continuous 変数、線形目的関数、線形の通常等式・不等式制約、および両方の最適化
sense です。非対応の変数 kind、非線形式、特殊制約は、#1084 で導入された構造化
{class}`~ommx.adapter.AdapterCompatibilityError` で報告されます。

移行後の constructor は Indicator、OneHot、SOS1 制約を暗黙に lower しません。
直接変換できない family は入力 {class}`~ommx.Instance` を変更せずに拒否されます。
意図した reformulation は呼び出し側で明示的に行い、prepared instance を再検査して
ください。

### 🆕 PySCIPOpt Adapter の native capability 宣言 ([#1086](https://github.com/Jij-Inc/ommx/pull/1086))

`OMMXPySCIPOptAdapter` は、Binary、Integer、Continuous 変数、二次の目的関数・
通常制約、線形 body の Indicator、native SOS1、および両方の最適化 sense からなる
一貫した native profile を宣言し、変換前に検査するようになりました。次数上限を
別々に持つため、二次の通常制約は受理しながら、二次の Indicator body は非対応として
正しく報告されます。

Adapter は OneHot 制約を暗黙に lower しなくなりました。OneHot、SemiInteger、
SemiContinuous、三次式、非線形 Indicator は、入力 {class}`~ommx.Instance` を
変更せずに {class}`~ommx.adapter.AdapterCompatibilityError` で報告されます。

### 🆕 Adapter Capability profile と互換性レポート ([#1084](https://github.com/Jij-Inc/ommx/pull/1084))

{meth}`~ommx.Instance.solver_requirements` で、solver が実際に扱う active な
入力全体の要件を導出できるようになりました。使用される変数の kind、目的関数と
制約の次数、制約の関係・family、最適化の sense が含まれます。
{class}`~ommx.CapabilityProfile` と {class}`~ommx.AdapterCapabilities` は、
native translator が直接受け取れる一貫した profile を表現し、構造化された
{class}`~ommx.PortableCompatibilityReport` を返します。
...
{class}`~ommx.adapter.SolverAdapter` の subclass は `CAPABILITIES` を宣言し、
`check_compatibility` または `require_compatible` で portable な比較と
adapter 固有の前提条件をまとめて検査できます。この検査は入力 instance を変更しません。
Profile が表すのは明示的な preparation 後に translator へ直接渡す入力であり、
relaxation や有限 penalty 変換による受理を native support として扱いません。
`ommx.v2.Feature` と `required_features` は独立した wire-format の Forward
Compatibility 概念であり、Adapter Capability の宣言ではありません。

### ⚠ Legacy Adapter Capability 語彙の削除 ([#1088](https://github.com/Jij-Inc/ommx/pull/1088))

Legacy な `AdditionalCapability`、`Capabilities`、`ADDITIONAL_CAPABILITIES`、
`required_capabilities`、`reduce_capabilities` API は削除されました。
{class}`~ommx.SpecialConstraintKind`、
{attr}`~ommx.Instance.active_special_constraint_kinds`、
{meth}`~ommx.Instance.lower_special_constraints` は、変換 selector を Adapter support の
宣言と見せることなく、特殊制約の対象を直接選択する明示的な lowering を
表現します。Adapter は lowered model を native profile に対して再検査しなければなりません。

### 🆕 OpenJij の native capability と明示的 preparation ([#1087](https://github.com/Jij-Inc/ommx/pull/1087))

{class}`~ommx_openjij_adapter.OMMXOpenJijSAAdapter` が native translator の
profile を厳密に宣言するようになりました。直接受け取るのは Binary 変数、任意次数の
多項式目的関数、active な制約なし、最小化の model です。native な
`check_compatibility` は Integer encoding、制約の penalty 化、sense 変換を solver
support として扱いません。

source model に明示的な変換が必要な場合は、`sample` または `solve` に
`preparation=True` を指定します。
{meth}`~ommx_openjij_adapter.OMMXOpenJijSAAdapter.check_preparation` と
{meth}`~ommx_openjij_adapter.OMMXOpenJijSAAdapter.prepare` の audit report は、
各 step を exact な書き換え、近似、有限 penalty に区別します。返される
{class}`~ommx.SampleSet` は source model に対して評価され、Experiment の sampling
にもその source `Instance` が記録されます。
...
Integer の log encoding で検査する 53-bit 上限や OpenJij 固有の制限は、adapter が
preparation に課す前提条件です。native capability でも `ommx.v2.Feature` でも
ありません。`Feature` は serialization の Forward Compatibility だけを担います。
native Spin support は OMMX に Spin variable kind を追加する
[#1082](https://github.com/Jij-Inc/ommx/issues/1082) の後に扱います。
...
### 🆕 Legacy `ADDITIONAL_CAPABILITIES` lowering API（廃止済み） ([#790](https://github.com/Jij-Inc/ommx/pull/790), [#805](https://github.com/Jij-Inc/ommx/pull/805), [#810](https://github.com/Jij-Inc/ommx/pull/810), [#811](https://github.com/Jij-Inc/ommx/pull/811), [#814](https://github.com/Jij-Inc/ommx/pull/814))

この subsection は 3.0.0 Alpha 2 で導入された API の記録です。その API は
現行 release より前に置き換えられており、`AdditionalCapability`、
`ADDITIONAL_CAPABILITIES`、constructor による自動 lowering は現行の Adapter Capability
model ではありません。

Alpha 2 では、Adapter が `ADDITIONAL_CAPABILITIES` クラス属性で特殊制約 family
を選択していました。`super().__init__(instance)` を呼ぶと、選択されていない特殊制約は
solver へ渡す前に通常制約へ変換されました（Indicator / SOS1 は Big-M、OneHot は
線形等式）。この時点の既存 Adapter にはその constructor 呼び出しの追加が必要でした。

現行 Adapter は `CAPABILITIES` で complete な native translator profile を宣言し、互換性のない
入力を変更せずに拒否します。特殊制約の明示的な lowering には
{class}`~ommx.SpecialConstraintKind` と {meth}`~ommx.Instance.lower_special_constraints` を使い、
その後 native compatibility を再検査します。これらの変換 selector は、serialization の
Forward Compatibility だけを担う `ommx.v2.Feature` とも無関係です。詳細は
[Adapter Capability モデルと明示的な preparation](../user_guide/capability_model.md) を参照してください。
