...
または anonymous image ref を Local Registry から削除できるようになりました。
CLI では `ommx rm <ref>` を使います。出力には、到達不能なデータが独立した
`ommx gc --delete` によって grace period 後に削除されるまで残ることも表示されます。
