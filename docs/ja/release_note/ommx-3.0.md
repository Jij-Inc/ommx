...
Experiment ref を、各 Experiment artifact を開かずに一覧できるようになりました。
返される `ExperimentRef` には image name、manifest digest、status、run/solve
数、更新時刻、manifest annotation が含まれます。任意の `prefix` は full image
reference 文字列に対して一致します。
