...
### 🆕 Local Registry からの Experiment 一覧 ([#1029](https://github.com/Jij-Inc/ommx/pull/1029))

`ommx.experiment.list_experiments()` で、SQLite Local Registry に保存された
Experiment refを一覧できるようになりました。返される`ExperimentRef`にはimage
name、manifest/config digest、status、run/solve数、更新時刻、manifest annotation、
Pythonの`dict`としての完全なExperiment configが含まれます。任意の`prefix`はfull
image reference文字列に対して一致します。
