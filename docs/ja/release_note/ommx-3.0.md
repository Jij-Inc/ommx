...
{meth}`~ommx.experiment.Experiment.run_parameters_df` では pandas の通常の
`float64` 値として復元されます。これにより、unbounded な比率や infeasibility
summary などの `+inf` / `-inf` 観測値は欠損セルと区別できます。`NaN` は `NaN`
として保持されますが、wide な pandas `float64` DataFrame では欠損 float セルも
`NaN` として表現されます。
