...
- {func}`Artifact.load_archive <ommx.artifact.Artifact.load_archive>` の意味論が変更: アーカイブをユーザの永続 SQLite Local Registry に **import** するようになりました (旧: レジストリ副作用無しの in-place 読み込み)。
