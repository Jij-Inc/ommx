...
### 🛠 Sample error を共通の Python boundary で通知 ([#1107](https://github.com/Jij-Inc/ommx/pull/1107))

`Samples.append` は、重複した sample ID を直接 `?` で共通の PyO3 error
boundary へ伝播し、`ValueError` として通知するようになりました。変更前に入力 ID
をすべて検証するため、append が失敗しても collection は変更されません。
...
range や、2 state に 3 sample を分配するような正しい partition も、integer
overflow や strategy panic なしで生成できます。State group 数 0 は sample 数も 0
の場合に限り有効です。

### 🛠 Model error を共通の Python boundary で通知 ([#1105](https://github.com/Jij-Inc/ommx/pull/1105))

Decision variable の追加と substitution、`Function.content_factor`、
OneHot/SOS1 の構築は、直接 `?` で Rust SDK の failure を共通の PyO3
error boundary へ伝播するようになりました。Decision variable / parameter
ID の衝突、不正な substitution、表現できない content factor、空の特殊制約は
一貫して `ValueError` になります。

Rust SDK の signal owner は boundary を越えても維持されます。重複 row は
`DecisionVariableError` のまま扱い、table 間の parameter 衝突と model 固有の
validation は、それぞれ対応する downcast 可能な signal を使用します。

### 🛠 Evaluation error を共通の Python boundary で通知 ([#1104](https://github.com/Jij-Inc/ommx/pull/1104))

Function、polynomial、constraint、named function、`Instance` の evaluation
API は、直接 `?` で Rust SDK の failure を共通の PyO3 error boundary へ
伝播するようになりました。特殊制約の伝播が必要な場合を含め、呼び出し側が
渡した不正な state は一貫して `ValueError` になり、内部の伝播 failure は
引き続き `RuntimeError` に fallback します。

Partial evaluation では、元の coefficient error を Rust の error chain に
保持するため、Python 側で個別に変換せず `ValueError` の分類を維持します。

### 🛠 残りの Python binding も共通の error boundary を使用 ([#1102](https://github.com/Jij-Inc/ommx/pull/1102))

`Instance`、`ParametricInstance`、attached metadata、random generator、
`Solution`、`Samples`、Artifact registry の残りの binding も、直接 `?` で
Rust SDK の failure を共通の PyO3 error boundary へ伝播するようになりました。
これにより、既存の型付き exception 分類が一貫して適用され、未分類の
SDK failure は引き続き `RuntimeError` に fallback します。

Python で発生した exception は変更せず伝播します。これには、
`Run.log_solve` と `Run.log_sample` から solver / sampler adapter が送出した
同一の exception object も含まれます。Binding が所有する component ID の
重複と、不足した penalty weight は `ValueError` として通知します。

### 🛠 Experiment error を共通の Python boundary で通知 ([#1101](https://github.com/Jij-Inc/ommx/pull/1101))

Experiment、Run、Solve、Sampling、local listing の binding は、直接 `?` で
Rust SDK の failure を共通の PyO3 error boundary へ伝播するようになりました。
Experiment または Run の attachment が存在しない場合は `KeyError` を送出します。
不正な image reference、autosave value、attachment media type、JSON input は
`ValueError`、registry、archive、storage、lifecycle の failure は
`RuntimeError` に fallback します。

Python codec、JSON callback、adapter、tracing hook、data library が送出した
exception は変更せず伝播します。Attachment codec は CAS blob を読む前に宣言した
media type を検証し、encode 結果には Python の `bytes` を要求します。Run body と
tracing cleanup が同時に失敗した場合も、元の body exception を保持し、Run は
failed または interrupted status で確実に閉じられます。

### 🛠 Artifact と Local Registry の error を一貫した exception として通知 ([#1100](https://github.com/Jij-Inc/ommx/pull/1100))

Artifact と Local Registry の binding は、entry point ごとに個別変換せず、共通の
PyO3 error boundary を通して Rust SDK の failure を伝播するようになりました。
不正な image reference、malformed digest、未対応または不正な layer media type、
存在しない typed layer、不正な OMMX payload は `ValueError` を送出します。有効な
digest で存在しない layer を参照した場合は `KeyError` を送出します。

Local Registry、archive、content-addressed storage の failure は、Local Registry
へ保存済みの不正な image ref を含め、引き続き `RuntimeError` に fallback します。
Python の JSON、NumPy、pandas codec が送出した exception は変更せず伝播します。
Descriptor は metadata-only のまま維持し、blob read は registry context を持つ
Artifact が所有します。

### 🛠 Parse failure を一貫して `ValueError` として通知 ([#1099](https://github.com/Jij-Inc/ommx/pull/1099))

Python の byte decode entry point に不正な protobuf payload を渡した場合、
OMMX message と field の context を保持した `ValueError` を送出するように
なりました。protobuf の semantic parse failure と不正な QPLIB syntax も、
共通の PyO3 error boundary を通して同じ exception を送出します。

Python の引数抽出 failure は、引き続き `TypeError` です。安定した OMMX-owned
parse signal がまだない file open failure と MPS parser failure は、実装依存の
error type や表示文字列から分類せず、従来どおり `RuntimeError` に fallback
します。

### 🛠 Structured SDK error を一貫した Python exception として通知 ([#1097](https://github.com/Jij-Inc/ommx/pull/1097))

Python binding は、安定した `DecisionVariableError`、`SolutionError`、
`SampleSetError` の signal を、共通の PyO3 error boundary で変換するように
なりました。structured result の参照で name、sample ID、constraint ID が
存在しない場合は `KeyError` を送出します。決定変数の bound が不正な場合、
subscript が重複する場合、parameter 付き constraint を抽出する場合、feasible
sample がない状態で best sample を要求した場合は、一貫して `ValueError` を
送出します。

`DecisionVariable` constructor に未知の整数 kind を渡した場合も、Python が
所有する入力検証として `ValueError` を送出します。Python native の failure は
そのまま維持され、分類されていない Rust SDK error は引き続き `RuntimeError` に
fallback します。

### 🛠 Numeric SDK error を一貫して `ValueError` として通知 ([#1096](https://github.com/Jij-Inc/ommx/pull/1096))

Python binding は、Rust SDK から直接返された `CoefficientError`、`AtolError`、
`BoundError` を、共通の PyO3 error boundary で変換するようになりました。
constructor、算術演算、binary power の縮約で生じる不正な係数、正でない許容誤差、
不正な bound は、entry point ごとの panic、`RuntimeError`、個別変換に分かれず、
一貫して `ValueError` を送出します。

係数 0 は従来どおり正常系として正規化されます。in-place 加算に失敗した場合も、
元の数値オブジェクトは変更されません。Python が所有する型検証や引数 shape の
検証も、それぞれの native exception を維持します。分類されていない Rust SDK の
失敗は、引き続き `RuntimeError` に fallback します。

### 🆕 remote Artifact lookup の型付き error ([#1090](https://github.com/Jij-Inc/ommx/pull/1090))

{meth}`~ommx.artifact.Artifact.load` と
{meth}`~ommx.experiment.Experiment.load` は、OCI transport の error を含む
汎用的な `RuntimeError` ではなく、OMMX が所有する exception として
remote lookup の失敗を通知するようになりました。exact ref が存在しない
場合だけを処理するには
{class}`~ommx.artifact.RemoteArtifactNotFoundError` を catch してください。
authentication、authorization、registry transport、invalid Artifact の失敗を
誤って「存在しない」と扱わずに済みます。すべての remote lookup exception は
{class}`~ommx.artifact.RemoteArtifactError` を継承します。
...
{class}`~ommx.artifact.RemoteArtifactTransportError`、
{class}`~ommx.artifact.InvalidRemoteArtifactError` も利用できます。exception の
message には元の registry / transport context が保持されます。両方の load
entry point は同じ PyO3 error conversion の境界を使用します。

### 🆕 構造制約の `VariableIDLike` 入力 ([#1078](https://github.com/Jij-Inc/ommx/pull/1078))

変数の identity だけが必要な構造制約の構築 API は、
`int | DecisionVariable | AttachedDecisionVariable` と定義される
`VariableIDLike` を受け取るようになりました。対象は
{class}`~ommx.OneHotConstraint`、{class}`~ommx.Sos1Constraint`、
{class}`~ommx.IndicatorConstraint`、および
{meth}`Constraint.with_indicator() <ommx.Constraint.with_indicator>` です。
...
{meth}`~ommx.Instance.log_encode` のように、本質的に ID の集合や mapping を扱う
API は従来どおり ID ベースです。
...
### ⚠ Experiment 専用 artifact type ([#1033](https://github.com/Jij-Inc/ommx/pull/1033))
...
### 🆕 Experiment Sampling record ([#1055](https://github.com/Jij-Inc/ommx/pull/1055))

{meth}`~ommx.experiment.Run.log_sample` で
{class}`~ommx.adapter.SamplerAdapter` を呼び出し、返された完全な
{class}`~ommx.SampleSet` を独立した {class}`~ommx.experiment.Sampling`
recordとして記録できるようになりました。sampling が成功していれば、SampleSet に
feasible sample がなくてもSamplingは `finished` になります。solver呼び出しは引き続き
{class}`~ommx.experiment.Solve` として記録され、outputは `Solution | None` です。
...
OMMX Artifact refを一覧できるようになりました。返される`ArtifactRef`にはimage
name、Manifest/Config digest、更新時刻、`artifactType`、Manifest annotation、
Pythonの`dict`としての完全なOCI Manifestが含まれます。
...
これに伴い`AnonymousArtifactRef.size`と`AnonymousArtifactRef.media_type`を削除しました。
descriptorのfieldはref一覧APIには含まれなくなります。

### 🆕 非有限 float の Run parameter ([#1043](https://github.com/Jij-Inc/ommx/pull/1043))

{meth}`~ommx.experiment.Run.log_parameter` は `float("inf")`、
`-float("inf")`、`float("nan")` を受け付けるようになりました。これらの値は
commit 済み Experiment Artifact を通して round-trip し、
{meth}`~ommx.experiment.Experiment.run_parameters_df` では pandas の nullable dtype
...
run-parameter table layer は、IEEE 754 の非有限値を保持できるよう JSON ではなく
MessagePack として保存します。個別の NaN payload bit は API の保証に含めません。
...
### 🆕 文脈付き Function formatting ([#1004](https://github.com/Jij-Inc/ommx/pull/1004), [#1011](https://github.com/Jij-Inc/ommx/pull/1011))
...
commit 済みの Experiment から新しい未 commit の Experiment を開始する {meth}`~ommx.experiment.Experiment.fork` を追加しました。fork 先は元の Experiment の attachments、Runs、Solves、Samplings、Run parameters を引き継ぎますが、親 Experiment は変更されません。fork 先で新しい Run や attachment を追加して commit すると、親の manifest descriptor が OCI `subject` として記録されます。
