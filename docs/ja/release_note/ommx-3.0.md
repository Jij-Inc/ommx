...
{meth}`Constraint.with_indicator() <ommx.v1.Constraint.with_indicator>` を使って既存の制約から {class}`~ommx.v1.IndicatorConstraint` を作成できます。PySCIPOpt Adapterはindicator constraintを含むインスタンスの求解をサポートしています:
