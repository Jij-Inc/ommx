...
### ⚠ PySCIPOpt Adapter の Input Class ([#1086](https://github.com/Jij-Inc/ommx/pull/1086))

`OMMXPySCIPOptAdapter` は、backend modelを直接構築する前に受け入れる入力を
`INPUT_CLASS` として宣言するようになりました。activeな数理内容で使われるBinary、
Integer、Continuous変数、二次以下の目的関数と通常の等式・不等式制約、線形bodyの
Indicator等式・不等式制約、SOS1制約、および両方の最適化senseを受け入れます。
OneHot制約は暗黙にlowerされません。class外の入力は変更されることなく
{class}`~ommx.adapter.AdapterNotApplicableError` として拒否され、このexception
にはclauseごとの構造化されたmismatchが含まれます。明示的にprepareした
{class}`~ommx.Instance` は別の入力であり、その値についてapplicabilityを再評価する
必要があります。

これはstable Python SDK 2.6.1からの公開exception契約の破壊的変更です。三次以上の
目的関数・通常制約や、使用中のSemiInteger・SemiContinuous変数は、従来
`OMMXPySCIPOptAdapterError` として拒否されていましたが、今後はbackend構築前に
{class}`~ommx.adapter.AdapterNotApplicableError` が送出されます。constructorでの
非対応入力の拒否をAdapter固有exceptionで捕捉していたコードは、
`AdapterNotApplicableError`を捕捉するか、構築前に`check_applicability()`を呼び出して
ください。stableで受け入れていた変数kindと多項式次数の範囲は変わらず、
`OMMXPySCIPOptAdapterError`は変換・backendのfailureで引き続き使用されます。

OneHotの暗黙lowering廃止とv3 first-class Indicator・SOS1制約のapplicability規則は、
...
{class}`~ommx.InstanceClassClause` は構造条件の論理積を1つ表し、class全体は
clauseの有限和です。membershipは渡された入力値そのものから評価され、
{class}`~ommx.InstanceClassMembershipReport` がclauseごとの構造化された
mismatchを返します。
...
{class}`~ommx.adapter.SolverAdapter` のsubclassは `INPUT_CLASS` を宣言し、
