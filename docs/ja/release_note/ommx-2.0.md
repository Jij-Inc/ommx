...
### Rustらしい `ParametricInstance` (2.0.3, [#566](https://github.com/Jij-Inc/ommx/pull/566))

`ParametricInstance` のPythonバインディングを、Protocol Buffersから自動生成された `ommx::v1::ParametricInstance` から新しいRustネイティブの `ommx::ParametricInstance` に移行しました。`Parse` トレイトによる厳密なバリデーションにより、未定義の変数を参照するなど意味的に不正なインスタンスをパース時に明確なエラーメッセージとともに拒否するようになりました。
...
`Instance.used_decision_variables` は目的関数や制約条件で実際に参照されている決定変数の集合を返します。`Instance.insert_constraint` による構築後の制約追加と、`Instance.penalty_method` による制約付き問題から無制約ペナルティ定式化への変換が可能になりました。
...
`Instance.partial_evaluate` 呼び出し時に、`OneHot` および `SOS1` の制約ヒントが固定された値を従属変数に自動的に伝播し、不動点まで反復するようになりました。これにより、部分評価後のダングリング変数参照によるアダプターエラーを防止します。
...
- (2.0.1) `substituted_value` プロパティとバイナリ冪の削減 ([#537](https://github.com/Jij-Inc/ommx/pull/537), [#540](https://github.com/Jij-Inc/ommx/pull/540))
- (2.0.1) `Bound` の値による比較 ([#541](https://github.com/Jij-Inc/ommx/pull/541))
- (2.0.2) Rust SDKで直接的な `from_bytes`/`to_bytes` ([#549](https://github.com/Jij-Inc/ommx/pull/549))
- (2.0.2) `Degree` に `PartialOrd<u32>` を実装 ([#550](https://github.com/Jij-Inc/ommx/pull/550))
- (2.0.3) QPLIBパーサーの更新 ([#575](https://github.com/Jij-Inc/ommx/pull/575))
- (2.0.6) `constraint_hints` サブモジュールの分離 ([#608](https://github.com/Jij-Inc/ommx/pull/608))
- (2.0.11) ID割り当てメソッド ([#650](https://github.com/Jij-Inc/ommx/pull/650))
...
- (2.0.1) `Instance.decision_variables_df` に `substituted_value` カラムが欠落 ([#542](https://github.com/Jij-Inc/ommx/pull/542))
- (2.0.3) `Instance` の MPS I/O、`Bound` での `f64::INFINITY` の処理 ([#562](https://github.com/Jij-Inc/ommx/pull/562), [#577](https://github.com/Jij-Inc/ommx/pull/577))
- (2.0.4) `Bound` で `-0.0` を `0.0` に正規化 ([#581](https://github.com/Jij-Inc/ommx/pull/581))
- (2.0.9) `to_qubo`/`to_hubo` のdocstring修正 ([#631](https://github.com/Jij-Inc/ommx/pull/631))
- (2.0.10) QPLIBアノテーションキーのフォーマット不整合 ([#648](https://github.com/Jij-Inc/ommx/pull/648))
