# Python SDK マイグレーションガイド

このページは、OMMX Python SDK のメジャーバージョン間マイグレーションで確認すべき変更点をまとめたものです。

- [v2 から v3 へ](#python-sdk-v2-to-v3-migration-guide) - 数学的なデータ型の PyO3 移行完了に伴う破壊的変更
- [v1 から v2 へ](#python-sdk-v1-to-v2-migration-guide) - Protocol Buffers 由来 API から PyO3 ベース API への移行

詳細な before / after コード例を英語版と照合したい場合は、[English version](https://jij-inc-ommx.readthedocs-hosted.com/en/latest/migration/python_sdk.html) も参照してください。
...
この節の基準バージョンは **v2.5.1**（tag `python-2.5.1`）です。それ以前の 2.x から移行する場合は、この節に加えて [v1 から v2 へ](#python-sdk-v1-to-v2-migration-guide) も確認してください。
