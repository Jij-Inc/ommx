...
`.from_protobuf()` / `.to_protobuf()` は、protobuf object と一緒に削除されています。`Instance` / `Solution` / `SampleSet` など全体をシリアライズする場合は、明示的な `from_v1_bytes` / `to_v1_bytes` または `from_v2_bytes` / `to_v2_bytes` を使います。
...
top-level root についても、protobuf version を名前に含まない `Instance.to_bytes()` / `Instance.from_bytes(...)`（および `ParametricInstance`、`Solution`、`SampleSet` の同名 API）は削除されています。legacy な v1 wire format には `to_v1_bytes()` / `from_v1_bytes(...)`、新しい v2 payload には `to_v2_bytes()` / `from_v2_bytes(...)` を使ってください。
