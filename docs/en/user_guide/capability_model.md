...
`solve_strict()` and `sample_strict()` APIs require an exact member. When an
application uses a custom Preparation policy, it explicitly copies and prepares
the input before calling the strict API.
