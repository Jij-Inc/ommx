...
the preparation-free `sample_strict()` / `solve_strict()` API.
...
samples = OMMXOpenJijSAAdapter.sample_strict(working)
```
...
applicability condition; `sample_strict()` enforces it without running
