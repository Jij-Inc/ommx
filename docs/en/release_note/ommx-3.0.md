...
- {func}`ArtifactBuilder.new_archive <ommx.artifact.ArtifactBuilder.new_archive>` → {func}`ArtifactDraft.new <ommx.artifact.ArtifactDraft.new>` + {func}`Artifact.save <ommx.artifact.Artifact.save>` (new method).
- {func}`ArtifactBuilder.new_archive_unnamed <ommx.artifact.ArtifactBuilder.new_archive_unnamed>` → {func}`ArtifactDraft.new_anonymous <ommx.artifact.ArtifactDraft.new_anonymous>` + `Artifact.save(path)`. Anonymous artifacts now carry a synthesized `<registry-id8>.ommx.local/anonymous:<timestamp>-<nonce>` image name instead of `None`.
